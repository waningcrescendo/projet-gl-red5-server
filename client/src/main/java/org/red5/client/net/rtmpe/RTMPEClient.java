 ...
 */
public class RTMPEClient extends RTMPClient
    implements INetStreamEventHandler,
   ...
   *
   * <p>{@link
   * http://help.adobe.com/en_US/FlashPlatform/reference/actionscript/3/mx/events/MetadataEvent.html}
   * {@link
   * http://help.adobe.com/en_US/FlashPlatform/reference/actionscript/3/mx/controls/VideoDisplay.html#metadata}
   *
      ...
      if (result instanceof ObjectMap) {
        ...
        if ("connect".equals(methodName)) {
          ...
          if (result instanceof Number) {
            ...
            // start: The default value is -2, which looks for a live stream, then a recorded
            // stream, and if it finds neither, opens a live stream.
            // If -1, plays only a live stream. If 0 or a positive number, plays a recorded stream,
            // beginning start seconds in.
            int start = -2;
            // duration: The default value is -1, which plays a live or recorded stream until it
            // ends. If 0, plays a single frame that is start seconds
            // from the beginning of a recorded stream. If a positive number, plays a live or
            // recorded stream for len seconds.
            int duration = -1;
