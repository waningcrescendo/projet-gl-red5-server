...
public class OutboundHandshake extends RTMPHandshake {
  ...
  public IoBuffer generateClientRequest1() {
    ...
    if (log.isTraceEnabled()) {
      log.trace(
          "Time and version handshake bytes: {}",
          Hex.encodeHexString(Arrays.copyOf(handshakeBytes, 8)));
    }
    ...
    if (fp9Handshake) {
      ...
      log.debug(
          "Digest is valid: {}",
          verifyDigest(digestPosClient, c1, RTMPHandshake.GENUINE_FP_KEY, 30));
    }
    if (log.isTraceEnabled()) {
  ...
  public IoBuffer decodeServerResponse1(IoBuffer in) {
    ...
    if (fp9Handshake) {
      ...
      if (useEncryption()) {
        ...
        switch (handshakeType) {
          case RTMPConnection.RTMP_ENCRYPTED:
            // update 'encoder / decoder state' for the RC4 keys. Both parties *pretend* as if
            // handshake part 2 (1536 bytes) was encrypted
            // effectively this hides / discards the first few bytes of encrypted session which is
            // known to increase the secure-ness of RC4
            // RC4 state is just a function of number of bytes processed so far that's why we just
            // run 1536 arbitrary bytes through the keys below
      ...
      calculateHMAC_SHA256(
          s1, digestPosServer, DIGEST_LENGTH, GENUINE_FP_KEY, GENUINE_FP_KEY.length, digestResp, 0);
      calculateHMAC_SHA256(
          c2,
          0,
      ...
      log.debug(
          ...
          Hex.encodeHexString(digestResp));
      // FP10 stuff
  ...
  public boolean decodeServerResponse2(byte[] s2) {
    ...
    if (fp9Handshake) {
      ...
      calculateHMAC_SHA256(
          c1, digestPosClient, DIGEST_LENGTH, GENUINE_FMS_KEY, GENUINE_FMS_KEY.length, digest, 0);
      calculateHMAC_SHA256(
          s2, 0, Constants.HANDSHAKE_SIZE - DIGEST_LENGTH, digest, DIGEST_LENGTH, signature, 0);
