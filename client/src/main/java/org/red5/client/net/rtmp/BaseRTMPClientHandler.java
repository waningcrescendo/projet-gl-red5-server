...
public abstract class BaseRTMPClientHandler extends BaseRTMPHandler implements IRTMPClient {
  ...
  /** Shared objects map */
  private volatile ConcurrentMap<String, ClientSharedObject> sharedObjects =
      new ConcurrentHashMap<>(1, 0.9f, 1);

  /** Net stream handling */
  private volatile CopyOnWriteArraySet<NetStreamPrivateData> streamDataList =
  ...
  public void connect(String server, int port, String application) {
    // log.debug("connect server: {} port {} application {}", new Object[] { server, port,
    // application });
  ...
  public void connect(
    ...
    // log.debug("connect server: {} port {} application {} connectCallback {}", new Object[] {
    // server, port, application, connectCallback });
    connect(server, port, makeDefaultConnectionParams(server, port, application), connectCallback);
  }

  /**
   * Creates the default connection parameters collection. Many implementations of this handler will
   * create a tcUrl if not found, it is created with the current server url.
   *
  ...
  public void connect(String server, int port, Map<String, Object> connectionParams) {
    // log.debug("connect server: {} port {} connectionParams {}", new Object[] { server, port,
    // connectionParams });
  ...
  public void connect(
    ...
    log.debug(
        ...
        new Object[] {
          server, port, connectionParams, connectCallback, Arrays.toString(connectCallArguments)
        });
  ...
  public void publish(Number streamId, String name, String mode, INetStreamEventHandler handler) {
    ...
    if (handler != null) {
      ...
      NetStreamPrivateData streamData =
          streamDataList.stream().filter(s -> s.getStreamId() == sid).findFirst().orElse(null);
      if (streamData != null) {
   ...
   *
   * @see <a
   *     href="https://web.archive.org/web/20150911224454/http://www.adobe.com/devnet/adobe-media-server/articles/dynstream_actionscript.html">ActionScript
  ...
  public void connectionOpened(RTMPConnection conn) {
    ...
    executor.submit(
        () -> {
          ...
          log.debug(
              "Writing 'connect' invoke: {}, invokeId: {}", invoke, invoke.getTransactionId());
  ...
  protected void onCommand(RTMPConnection conn, Channel channel, Header source, ICommand command) {
    ...
    if (onStatus) {
      log.debug("onStatus");
      Number streamId =
          (Number) (source.getStreamId() != null ? source.getStreamId().doubleValue() : 1.0d);
      ...
      if (streamId != null) {
        ...
        if (streamData == null) {
          log.warn(
              "Stream data was null for id: {} entry count: {}", streamId, streamDataList.size());
    ...
    if (serviceProvider == null) {
      ...
      log.info(
          "No service provider / method for: {}; to handle calls like onBWCheck, add a service provider",
          methodName);
    } else {
        ...
        if (callResult instanceof Number) {
          ...
          if (map.containsKey("streamId")) {
            ...
            if (tmpStreamId instanceof Number) {
              ...
              log.warn(
                  "CreateStreamCallBack resultReceived - stream id is not a number: {}",
                  tmpStreamId);
            }
          }
        }
        log.debug(
            "CreateStreamCallBack resultReceived - stream id: {} call: {} connection: {}",
            streamId,
            call,
            conn);
    ...
    public void resultReceived(IPendingServiceCall call) {
      ...
      if (callResult != null) {
        ...
        final Number streamId =
            (Number)
                (callResult instanceof Number
                    ? callResult
                    : (callResult instanceof Map ? ((Map<?, ?>) callResult).get("streamId") : 1.0));
        log.debug(
            "DeleteStreamCallBack resultReceived - stream id: {} call: {} connection: {}",
            streamId,
            call,
            conn);
        if (conn != null) {
