...
public class RTMPEIoFilter extends IoFilterAdapter {
  ...
  @Override
  public void messageReceived(NextFilter nextFilter, IoSession session, Object obj)
      throws Exception {
    String sessionId = (String) session.getAttribute(RTMPConnection.RTMP_SESSION_ID);
    log.trace("Session id: {}", sessionId);
    RTMPMinaConnection conn =
        (RTMPMinaConnection)
            RTMPClientConnManager.getInstance().getConnectionBySessionId(sessionId);
    if (conn == null) {
    ...
    ReadBuffer buffer =
        Optional.ofNullable((ReadBuffer) session.getAttribute(RTMPConnection.RTMP_BUFFER))
            .orElseGet(
                () -> {
                  // add the attribute
    ...
    OutboundHandshake handshake =
        session.containsAttribute(RTMPConnection.RTMP_HANDSHAKE)
            ? (OutboundHandshake) session.getAttribute(RTMPConnection.RTMP_HANDSHAKE)
            : null;
        ...
        if (s0s1Size >= (Constants.HANDSHAKE_SIZE + 1)) {
          ...
          if (handshake != null) {
            if (handshake.getHandshakeType() != connectionType) {
              log.debug(
                  "Server requested handshake type: {} client requested: {}",
                  connectionType,
                  handshake.getHandshakeType());
            }
   ...
   * @param handshake
   */
  private static void completeConnection(
      IoSession session, RTMPMinaConnection conn, OutboundHandshake handshake) {
    ...
    session
        .getFilterChain()
        .addAfter(
            "rtmpeFilter", "protocolFilter", new ProtocolCodecFilter(new RTMPMinaCodecFactory()));
    // let the client know it may proceed
    BaseRTMPClientHandler handler =
        (BaseRTMPClientHandler) session.getAttribute(RTMPConnection.RTMP_HANDLER);
