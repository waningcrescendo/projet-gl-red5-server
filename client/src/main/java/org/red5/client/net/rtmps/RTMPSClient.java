...
public class RTMPSClient extends RTMPClient {
  ...
  /**
   * Creates a new RTMPSClient with the given keystore type, password, and paths to store files. If
   * the stores are inside a jar file, use the following format:
   * jar:file:/path/to/your.jar!/path/to/file/in/jar
   *
   * @param keyStoreType keystore type
   * @param password keystore password
   ...
   */
  public RTMPSClient(
      String keyStoreType, String password, String keystorePath, String truststorePath)
      throws IOException {
    protocol = "rtmps";
    this.keyStoreType = keyStoreType;
    this.password = password.toCharArray();
      ...
      if (keystoreStream != null && truststoreStream != null) {
        context =
            TLSFactory.getTLSContext(
                keyStoreType, password, keystoreStream, password, truststoreStream);
      } else {
