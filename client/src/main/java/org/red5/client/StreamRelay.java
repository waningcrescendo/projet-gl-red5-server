...
public class StreamRelay {
  ...
  public static void main(String... args) {
    ...
    if (args == null || args.length < 7) {
      System.out.println(
          "Not enough args supplied. Usage: <source uri> <source app> <source stream name> <destination uri> <destination app> <destination stream name> <publish mode>");
    } else {
      // parse the args
      ...
      proxy.init();
      proxy.setConnectionClosedHandler(
          new Runnable() {
            @Override
            public void run() {
      ...
      Map<String, Object> defParams =
          client.makeDefaultConnectionParams(sourceHost, sourcePort, sourceApp);
  ...
  private static final class CreateStreamCallback implements IPendingServiceCallback {
    ...
    public void resultReceived(IPendingServiceCall call) {
      ...
      if (sourceStreamName.endsWith(".flv")
          || sourceStreamName.endsWith(".f4v")
