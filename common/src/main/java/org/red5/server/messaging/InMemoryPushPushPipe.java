...
public class InMemoryPushPushPipe extends AbstractPipe {
  ...
  public boolean subscribe(IConsumer consumer, Map<String, Object> paramMap) {
    if (consumer instanceof IPushableConsumer) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "Consumer subscribe{} {} params: {}",
            new Object[] {(success ? "d" : " failed"), consumer, paramMap});
      }
      if (success) {
        fireConsumerConnectionEvent(
            consumer, PipeConnectionEvent.EventType.CONSUMER_CONNECT_PUSH, paramMap);
      }
      return success;
    } else {
  ...
  public boolean subscribe(IProvider provider, Map<String, Object> paramMap) {
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Provider subscribe{} {} params: {}",
          new Object[] {(success ? "d" : " failed"), provider, paramMap});
    }
    if (success) {
      fireProviderConnectionEvent(
          provider, PipeConnectionEvent.EventType.PROVIDER_CONNECT_PUSH, paramMap);
    }
    return success;
  }
   ...
   * @throws IOException In case IOException of some sort is occurred
   */
