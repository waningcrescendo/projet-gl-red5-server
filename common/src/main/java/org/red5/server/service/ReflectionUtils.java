...
public class ReflectionUtils {
  ...
  /**
   * Returns (method, params) for the given service or method name if found on a service or scope
   * handler. There is no connection argument for these calls.
   *
   * <p>SharedObjectScope uses this method to find methods on the handler.
   *
  ...
  public static Object[] findMethod(Object service, String methodName, List<?> listArgs) {
    ...
    final Set<Method> methods =
        Arrays.stream(service.getClass().getMethods())
            .filter(m -> (m.getName().equals(methodName) && !m.getName().contains("$")))
            .filter(m -> m.getParameterCount() == 1 || m.getParameterCount() == listArgs.size())
      ...
      for (Method method : methods) {
        ...
        if (paramCount == callParams.length) {
          ...
          try {
            ...
            log.warn(
                "Method {} not found in {} with parameters {}",
                methodName,
                service,
  ...
  public static Object[] findMethod(
      IConnection conn, IServiceCall call, Object service, String methodName) {
    if (isDebug) {
      log.debug(
          "Find method: {} in service: {} for call: {} and connection: {}",
          methodName,
          service,
          call,
          conn);
    ...
    // XXX(paul) someday this will be deprecated as its an extremely legacy feature to have a method
    // with a
            ...
            .filter(
                m ->
                    m.getParameterCount() == 1
                        || m.getParameterCount() == callParams.length
                        || m.getParameterCount() == (callParams.length + 1))
            .collect(Collectors.toUnmodifiableSet());
    if (methods.isEmpty()) {
      ...
      for (Method method : methods) {
        ...
        if (conn != null
            && paramCount == (callParams.length + 1)
            && paramTypes[0].isAssignableFrom(IConnection.class)) {
          // attempt to convert the args to match the method
          try {
