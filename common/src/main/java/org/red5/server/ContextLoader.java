 ...
 */
@ManagedResource(
    objectName = "org.red5.server:name=contextLoader,type=ContextLoader",
    description = "ContextLoader")
public class ContextLoader
    implements ApplicationContextAware, InitializingBean, DisposableBean, ContextLoaderMXBean {
  ...
  /**
   * Loads a context (Red5 application) and stores it in a context map, then adds it's beans to
   * parent (that is, Red5)
   *
  ...
  public void loadContext(String name, String config) {
    ...
    if (parentContext == null) {
      log.debug(
          "Lookup common - bean:{} local:{} singleton:{}",
          new Object[] {
  ...
  /**
   * Unloads a context (Red5 application) and removes it from the context map, then removes it's
   * beans from the parent (that is, Red5)
   *
   * @param name Context name
   */
  @SuppressWarnings("null")
  public void unloadContext(String name) {
    ...
    if (factory.containsSingleton(name)) {
      ...
      FileSystemXmlApplicationContext ctx =
          (FileSystemXmlApplicationContext) factory.getSingleton(name);
      if (ctx.isRunning()) {
