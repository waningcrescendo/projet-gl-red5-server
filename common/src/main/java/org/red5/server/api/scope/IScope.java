 ...
 * The scope object.
 *
 * <p>A stateful object shared between a group of clients connected to the same context path. Scopes
 * are arranged in hierarchical way, so its possible for a scope to have a parent and children
 * scopes. If a client connects to a scope then they are also connected to its parent scope. The
 * scope object is used to access resources, shared object, streams, etc. Scope is a general option
 * for grouping things in an application.
 *
...
public interface IScope extends IBasicScope, ResourcePatternResolver, IServiceHandlerProvider {
  ...
  /**
   * Creates child scope with name given and returns success value. Returns true on success and
   * false if given scope already exists among children.
   *
  ...
  /**
   * Adds scope as a child scope. Returns true on success and false if given scope is already a
   * child of current.
   *
