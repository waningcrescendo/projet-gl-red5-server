...
public class RTMPHandler extends BaseRTMPHandler {
      ...
      if (scope.hasHandler()) {
        ...
        if (!handler.serviceCall(conn, call)) {
          ...
          log.warn(
              ...
              scope.getName(),
              new Exception("Service call failed"));
          return;
        }
      }
  ...
  protected void onCommand(RTMPConnection conn, Channel channel, Header source, ICommand command) {
            ...
            IStreamService streamService =
                (IStreamService)
                    ScopeUtils.getScopeService(
                        conn.getScope(), IStreamService.class, StreamService.class);
            try {
              log.debug(
                  "Invoking {} from {} with service: {}",
                  new Object[] {call, conn.getSessionId(), streamService});
              if (invokeCall(conn, call, streamService)) {
                ...
                status.setDescription(
                    String.format("Failed to %s (stream id: %d)", action, source.getStreamId()));
              ...
              status.setDescription(
                  String.format(
                      "Error while invoking %s (stream id: %d)", action, source.getStreamId()));
          ...
          try {
            ...
            if (scope != null) {
              if (isDebug) {
                ...
                log.debug(
                    "Conn {}, scope {}, call {} args {}",
                    new Object[] {conn, scope, call, call.getArguments()});
              }
    ...
    if (command instanceof Invoke) {
      ...
      if ((source.getStreamId().intValue() != 0)
  ...
  private void sendSOCreationFailed(RTMPConnection conn, SharedObjectMessage message) {
    ...
    if (conn.isChannelUsed(3)) {
      ...
      log.warn(
          "Channel is not in-use and cannot handle SO event: {}",
          message,
          new Exception("SO event handling failure"));
      // XXX Paul: I dont like this direct write stuff, need to move to event-based
      conn.getChannel(3).write(message);
    }
  }
  ...
  protected void onSharedObject(
      RTMPConnection conn, Channel channel, Header source, SharedObjectMessage message) {
    if (isDebug) {
      log.debug(
          "onSharedObject - conn: {} channel: {} so message: {}",
          new Object[] {conn.getSessionId(), channel.getId(), message});
    }
    final IScope scope = conn.getScope();
    if (scope != null) {
      // so name
      ...
      if (so != null) {
        if (so.isPersistent() == persistent) {
          ...
          log.warn(
              "Shared object persistence mismatch - current: {} incoming: {}",
              so.isPersistent(),
              persistent);
          // reset the object so we can re-use it
          message.reset();
        ...
        // add the error event
        message.addEvent(
            new SharedObjectEvent(ISharedObjectEvent.Type.CLIENT_STATUS, "error", NC_CALL_FAILED));
        conn.getChannel(3).write(message);
      }
    } else {
