 ...
 * calls, bandwidth configuration, AMF encoding type (AMF0/AMF3), connection state (is alive, last
...
public abstract class RTMPConnection extends BaseConnection
  ...
  private static final AtomicIntegerFieldUpdater<RTMPConnection> receivedQueueSizeUpdater =
      AtomicIntegerFieldUpdater.newUpdater(RTMPConnection.class, "receivedQueueSize");
  ...
  protected transient ConcurrentMap<Integer, Channel> channels =
      new ConcurrentHashMap<>(channelsInitalCapacity, 0.9f, channelsConcurrencyLevel);
  ...
  protected transient ConcurrentMap<Number, IClientStream> streams =
      new ConcurrentHashMap<>(streamsInitalCapacity, 0.9f, streamsConcurrencyLevel);
  ...
  protected transient Set<Number> reservedStreams =
      Collections.newSetFromMap(
          new ConcurrentHashMap<Number, Boolean>(
              reservedStreamsInitalCapacity, 0.9f, reservedStreamsConcurrencyLevel));
  ...
  protected transient ConcurrentMap<Integer, IPendingServiceCall> pendingCalls =
      new ConcurrentHashMap<>(pendingCallsInitalCapacity, 0.75f, pendingCallsConcurrencyLevel);
  ...
  /**
   * Queue for received RTMP packets. This is a transfer queue from which packets are passed to a
   * handler.
   */
  ...
  private void startRoundTripMeasurement() {
    if (scheduler != null) {
      if (pingInterval > 0) {
        ...
        try {
          // schedule with an initial delay of now + 2s to prevent ping messages during connect post
          // processes
          ...
          if (delayUntilDate != null) {
            keepAliveTask =
                scheduler.scheduleWithFixedDelay(new KeepAliveTask(), delayUntilDate, pingInterval);
          }
          if (isDebug) {
      ...
      // reducing from error to trace as its not all that important of a message these days to have
      // such promotion
      log.trace(
          "startRoundTripMeasurement not enabled. If RTMP, can occur when lost before handshake is complete");
    }
  }
  ...
  public boolean isValidStreamId(Number streamId) {
    ...
    if (isTrace) {
      log.trace(
          "Checking validation for streamId {}; reservedStreams: {}; streams: {}, connection: {}",
          new Object[] {d, reservedStreams, streams, sessionId});
    }
    ...
    if (streams.get(d) != null) {
      ...
      log.warn(
          "Another stream already exists with this id in streams {} in connection: {}",
          streams,
          sessionId);
      return false;
    }
  ...
  public ISingleItemSubscriberStream newSingleItemSubscriberStream(Number streamId) {
    if (isValidStreamId(streamId)) {
      ...
      SingleItemSubscriberStream siss =
          (SingleItemSubscriberStream) scope.getContext().getBean("singleItemSubscriberStream");
  ...
  public IClientStream getStreamByChannelId(int channelId) {
    ...
    if (isTrace) {
      log.trace(
          "Stream requested for channel id: {} stream id: {} streams: {}",
          channelId,
          streamId,
          streams);
    }
  ...
  /**
   * Creates output stream object from stream id. Output stream consists of audio, video, and data
   * channels.
   *
  ...
  private boolean registerStream(IClientStream stream) {
    ...
    log.error(
        "Unable to register stream {}, stream with id {} was already added",
        stream,
        stream.getStreamId());
    return false;
  }
  ...
  public void close() {
    if (closing.compareAndSet(false, true)) {
      ...
      IStreamService streamService =
          (IStreamService)
              ScopeUtils.getScopeService(scope, IStreamService.class, StreamService.class);
      if (streamService != null) {
        ...
        if (isDebug) {
          log.debug(
              "Stream service was not found for scope: {}",
              (scope != null ? scope.getName() : "null or non-existant"));
        }
      }
      ...
      if (isTrace) {
        // dump memory stats
        log.trace(
            "Memory at close - free: {}K total: {}K",
            Runtime.getRuntime().freeMemory() / 1024,
            Runtime.getRuntime().totalMemory() / 1024);
      }
              ...
              () -> {
                ...
                try {
                  do {
                    ...
                    Packet p =
                        receivedPacketQueue.poll(
                            maxPollTimeout, TimeUnit.MILLISECONDS); // wait for packet with timeout
                    if (p != null) {
                      if (isTrace) {
                      ...
                      try {
                        ...
                        ListenableFuture<Packet> future =
                            (ListenableFuture<Packet>)
                                executor.submitListenable(new ListenableFutureTask<Packet>(task));
                        future.addCallback(
                            new ListenableFutureCallback<Packet>() {
                              ...
                              public void onFailure(Throwable t) {
                                log.warn(
                                    "onFailure - processingTime: {} msgtype: {} task: {}",
                                    getProcessingTime(),
                                    getMessageType(packet),
                                    task);
                              }
                              ...
                              public void onSuccess(Packet packet) {
                                log.debug(
                                    "onSuccess - processingTime: {} msgtype: {} task: {}",
                                    getProcessingTime(),
                                    getMessageType(packet),
                                    task);
                              }
                        ...
                        if (isDebug) {
                          log.debug(
                              "Execution rejected on {} - {} lock permits - decode: {} encode: {}",
                              getSessionId(),
                              RTMP.states[getStateCode()],
                              decoderLock.availablePermits(),
                              encoderLock.availablePermits());
                        }
                      }
                    }
                  } while (state.getState()
                      < RTMP.STATE_ERROR); // keep processing unless we pass the error state
                } catch (InterruptedException e) {
                  log.debug(
                      "Interrupted while waiting for message {} state: {}",
                      sessionId,
                      RTMP.states[getStateCode()],
                      e);
                } catch (Exception e) {
                  log.error(
                      "Error processing received message {} state: {}",
                      sessionId,
                      RTMP.states[getStateCode()],
                      e);
                } finally {
  ...
  public void messageSent(Packet message) {
    ...
    if (event instanceof VideoData) {
      ...
      if (isTrace) {
        log.trace(
            "Stream id: {} pending: {} total pending videos: {}",
            streamId,
            pending,
            pendingVideos.size());
      }
  ...
  public long getPendingVideoMessages(Number streamId) {
    ...
    if (isTrace) {
      log.trace(
          "Stream id: {} pendingCount: {} total pending videos: {}",
          streamId,
          pendingCount,
          pendingVideos.size());
    }
   ...
   */
  public void sendSharedObjectMessage(
      String name, int currentVersion, boolean persistent, Set<ISharedObjectEvent> events) {
    // create a new sync message for every client to avoid concurrent access through multiple
    // threads
    SharedObjectMessage syncMessage =
        state.getEncoding() == Encoding.AMF3
  ...
  public void pingReceived(Ping pong) {
    ...
    if (isDebug) {
      log.debug(
          "Pong received: session=[{}] at {} with value {}, previous received at {}",
          new Object[] {getSessionId(), now, pongValue, previousPingValue});
    }
    if (pongValue == previousPingValue) {
      ...
      if (isDebug) {
        log.debug(
            ...
            new Object[] {getSessionId(), lastPingRoundTripTime.get()});
      }
    } else {
      ...
      if (getPendingMessages() > 4) {
        ...
        log.info(
            ...
            new Object[] {getSessionId(), pingRtt});
      }
    }
  ...
  public String toString() {
    if (isDebug) {
      ...
      return String.format(
          "%1$s %2$s:%3$s to %4$s client: %5$s session: %6$s state: %7$s",
          new Object[] {
                ...
                if (lastPongTime > 0
                  ...
                  log.warn(
                      ...
                      new Object[] {
                        ...
                        (now - lastPingTime),
                        (now - lastBytesReadTime)
                      });
                  // the following line deals with a very common support request
                  log.warn(
                      "Client on session=[{}] has not responded to our ping for [{} ms] and we haven't received data for [{} ms]",
                      new Object[] {
                        getSessionId(), (lastPingTime - lastPongTime), (now - lastBytesReadTime)
                      });
                  onInactive();
                } else {
  ...
  private class WaitForHandshakeTask implements Runnable {
    ...
    public WaitForHandshakeTask() {
      if (isTrace) {
        log.trace(
            "WaitForHandshakeTask created on scheduler: {} for session: {}",
            scheduler,
            getSessionId());
      }
    }
