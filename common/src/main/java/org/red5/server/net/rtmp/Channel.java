...
public class Channel {
  ...
  public void write(IRTMPEvent event) {
    if (!connection.isClosed()) {
      ...
      if (id > 3 && stream == null) {
        log.warn(
            "Non-existant stream for channel id: {}, session: {} discarding: {}",
            id,
      ...
      log.debug(
          "Connection {} is closed, cannot write to channel: {}", connection.getSessionId(), id);
    }
  }
  ...
  public void sendStatus(Status status) {
    if (connection != null) {
      ...
      if (andReturn) {
        ...
        if (status.getCode().equals(StatusCodes.NS_PLAY_START)) {
          ...
          if (scope
              ...
              .containsBean(IRtmpSampleAccess.BEAN_NAME)) {
            IRtmpSampleAccess sampleAccess =
                (IRtmpSampleAccess)
                    scope.getContext().getApplicationContext().getBean(IRtmpSampleAccess.BEAN_NAME);
            ...
            if (videoAccess || audioAccess) {
              ...
              notify.setData(
                  IoBuffer.wrap(
                      new byte[] {
                        0x01,
                        (byte) (audioAccess ? 0x01 : 0x00),
                        0x01,
                        (byte) (videoAccess ? 0x01 : 0x00)
                      }));
      ...
      // send directly to the corresponding stream as for some status codes, no stream has been
      // created  and thus "getStreamByChannelId" will fail
  ...
  public String toString() {
    if (connection != null) {
      return "Channel [id="
          + id
          + ", stream id="
          + connection.getStreamIdForChannelId(id)
          + ", session="
          + connection.getSessionId()
          + "]";
    }
