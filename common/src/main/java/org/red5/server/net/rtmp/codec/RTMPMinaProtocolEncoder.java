    ...
    if (!session.containsAttribute(RTMPConnection.RTMP_HANDSHAKE)) {
      ...
      if (conn != null) {
        ...
        if (!conn.equals(localConn)) {
          if (localConn != null) {
            log.debug(
                "Connection local ({}) didn't match io session ({})",
                localConn.getSessionId(),
                sessionId);
          }
        ...
        try {
          // get the buffer
          final IoBuffer buf =
              message instanceof IoBuffer ? (IoBuffer) message : encoder.encode(message);
          if (buf != null) {
            int requestedWriteChunkSize = conn.getState().getWriteChunkSize();
            log.trace(
                "Requested chunk size: {} target chunk size: {}",
                requestedWriteChunkSize,
                targetChunkSize);
            if (buf.remaining() <= targetChunkSize * 2) {
              ...
              int sentChunks =
                  Chunker.chunkAndWrite(out, buf, requestedWriteChunkSize, targetChunkSize);
  ...
  private static final class Chunker {
    ...
    public static LinkedList<IoBuffer> chunk(IoBuffer message, int chunkSize, int desiredSize) {
      ...
      do {
        ...
        log.trace(
            "Length: {} remaining: {} pos+len: {} limit: {}",
            new Object[] {length, remaining, (message.position() + length), limit});
        if (length > remaining) {
          length = remaining;
        }
        // add a chunk
    ...
    public static int chunkAndWrite(
        ProtocolEncoderOutput out, IoBuffer message, int chunkSize, int desiredSize) {
      int sentChunks = 0;
      int targetSize = desiredSize > chunkSize ? desiredSize : chunkSize;
      int limit = message.limit();
      do {
