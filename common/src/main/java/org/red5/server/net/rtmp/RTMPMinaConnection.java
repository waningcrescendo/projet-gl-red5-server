...
public class RTMPMinaConnection extends RTMPConnection implements RTMPMinaConnectionMXBean {
                ...
                if (future.isClosed()) {
                  ...
                  if (log.isTraceEnabled()) {
                    log.trace(
                        ...
                        getSessionId(),
                        (String) ioSession.getAttribute(RTMPConnection.RTMP_SESSION_ID));
                  }
  ...
  public boolean isIdle() {
    ...
    if (!idle) {
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug(
            "Connection idle - read: {} write: {}",
            ioSession.isReaderIdle(),
            ioSession.isWriterIdle());
        if (log.isTraceEnabled()) {
          log.trace(
              "Session - write queue: {} session count: {}",
      ...
      if (ioTime >= maxInactivity) {
        log.warn(
            "Connection {} has exceeded the max inactivity threshold of {} ms",
            getSessionId(),
            maxInactivity);
        log.debug(
            "Prepared to clear write queue, if session is connected: {}; closing? {}",
    ...
    if (ioSession != null) {
      ...
      while (state.getState() < RTMP.STATE_ERROR) {
        ...
        try {
          ...
          log.warn(
              "Interrupted while waiting for write lock (writeRaw). State: {}",
              RTMP.states[state.getState()],
              e);
  ...
  protected void registerJMX() {
    ...
    try {
      ...
      oName =
          new ObjectName(
              String.format(
                  "org.red5.server:type=%s,connectionType=%s,host=%s,port=%d,clientId=%s",
                  cName, type, hostStr, port, client.getId()));
  ...
  public void dumpInfo() {
    log.trace(
        "{} session: {} state: {} keep-alive running: {}",
        new Object[] {
          getClass().getSimpleName(), getSessionId(), RTMP.states[getStateCode()], running
        });
    log.trace(
        "Decoder lock - permits: {} queue length: {}",
        decoderLock.availablePermits(),
        decoderLock.getQueueLength());
    log.trace(
        "Encoder lock - permits: {} queue length: {}",
        encoderLock.availablePermits(),
        encoderLock.getQueueLength());
