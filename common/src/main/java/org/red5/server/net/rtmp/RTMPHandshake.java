...
/**
 * Generates and validates the RTMP handshake response for Flash Players. Client versions equal to
 * or greater than Flash 9,0,124,0 require a nonzero value as the fifth byte of the handshake
 * request.
 *
...
public abstract class RTMPHandshake implements IHandshake {
  ...
  protected void initRC4Encryption(byte[] sharedSecret) {
    ...
    log.debug(
        "Outgoing public key [{}]: {}",
        outgoingPublicKey.length,
        Hex.encodeHexString(outgoingPublicKey));
    byte[] rc4keyOut = new byte[32];
    // digest is 32 bytes, but our key is 16
    calculateHMAC_SHA256(
        outgoingPublicKey, 0, outgoingPublicKey.length, sharedSecret, KEY_LENGTH, rc4keyOut, 0);
    ...
    log.debug(
        "Incoming public key [{}]: {}",
        incomingPublicKey.length,
        Hex.encodeHexString(incomingPublicKey));
    // digest is 32 bytes, but our key is 16
    byte[] rc4keyIn = new byte[32];
    calculateHMAC_SHA256(
        incomingPublicKey, 0, incomingPublicKey.length, sharedSecret, KEY_LENGTH, rc4keyIn, 0);
  ...
  protected byte[] getSharedSecret(byte[] publicKeyBytes, KeyAgreement agreement) {
    ...
    try {
      ...
      KeySpec otherPublicKeySpec =
          new DHPublicKeySpec(otherPublicKeyInt, RTMPHandshake.DH_MODULUS, RTMPHandshake.DH_BASE);
  ...
  public void calculateDigest(
    ...
    if (log.isTraceEnabled()) {
      log.trace(
          "calculateDigest - digestPos: {} handshakeOffset: {} keyLen: {} digestOffset: {}",
          digestPos,
          handshakeOffset,
          keyLen,
          digestOffset);
    ...
    // copy bytes from handshake message starting at handshake offset into message
    // start at index 0
    // and up-to digest position length
    System.arraycopy(handshakeMessage, handshakeOffset, message, 0, digestPos);
    // copy bytes from handshake message starting at handshake offset plus digest
    // position plus
    // digest length
    // into message start at digest position and up-to message length minus digest
    // position
    System.arraycopy(
        handshakeMessage,
        handshakeOffset + digestPos + DIGEST_LENGTH,
        message,
        digestPos,
  ...
  public boolean verifyDigest(int digestPos, byte[] handshakeMessage, byte[] key, int keyLen) {
    if (log.isTraceEnabled()) {
      log.trace(
          "verifyDigest - digestPos: {} keyLen: {} handshake size: {} ",
          digestPos,
          keyLen,
    ...
    if (!Arrays.equals(
        Arrays.copyOfRange(handshakeMessage, digestPos, (digestPos + DIGEST_LENGTH)), calcDigest)) {
      return false;
    }
    return true;
  }
   ...
   * @param digestOffset digest offset
   */
  public void calculateHMAC_SHA256(
      byte[] message,
      int messageOffset,
      int messageLen,
      byte[] key,
    ...
    if (log.isTraceEnabled()) {
      log.trace(
          "calculateHMAC_SHA256 - messageOffset: {} messageLen: {}", messageOffset, messageLen);
      log.trace(
          "calculateHMAC_SHA256 - message: {}",
          Hex.encodeHexString(
              Arrays.copyOfRange(message, messageOffset, messageOffset + messageLen)));
      log.trace(
          "calculateHMAC_SHA256 - keyLen: {} key: {}",
          keyLen,
          Hex.encodeHexString(Arrays.copyOf(key, keyLen)));
      // log.trace("calculateHMAC_SHA256 - digestOffset: {} digest: {}", digestOffset,
      // Hex.encodeHexString(Arrays.copyOfRange(digest, digestOffset, digestOffset +
      // DIGEST_LENGTH)));
    }
    byte[] calcDigest;
    try {
  ...
  public void calculateSwfVerification(byte[] handshakeMessage, byte[] swfHash, int swfSize) {
    // SHA256 HMAC hash of decompressed SWF, key are the last 32 bytes of the server
    // handshake
    byte[] swfHashKey = new byte[DIGEST_LENGTH];
    System.arraycopy(
        handshakeMessage, Constants.HANDSHAKE_SIZE - DIGEST_LENGTH, swfHashKey, 0, DIGEST_LENGTH);
    byte[] bytesFromServerHash = new byte[DIGEST_LENGTH];
    calculateHMAC_SHA256(
        swfHash, 0, swfHash.length, swfHashKey, DIGEST_LENGTH, bytesFromServerHash, 0);
    ...
    log.debug(
        ...
        swfSize,
        Hex.encodeHexString(swfHash),
        Hex.encodeHexString(swfVerificationBytes));
  }
   ...
   * @param array array to get signature
   * @param offset offset to start from
   * @param keyId index of Blowfish key
   */
  ...
  /**
   * Gets the DH offset in the handshake bytes array based on validation scheme Generates DH keypair
  ...
  /**
   * Returns the contained handshake bytes. These are just random bytes if the player is using an
   * non-versioned player.
   *
   * @return cipher
   */
