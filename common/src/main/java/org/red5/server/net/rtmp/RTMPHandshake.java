...
/**
 * Generates and validates the RTMP handshake response for Flash Players. Client
 * versions equal to
 * or greater than Flash 9,0,124,0 require a nonzero value as the fifth byte of
 * the handshake
 * request.
 *
...
public abstract class RTMPHandshake implements IHandshake {
  ...
  /**
   * Sets the handshake type. Currently only two types are supported, plain and
   * encrypted.
   *
  ...
  /**
   * Returns the contained handshake bytes. These are just random bytes if the
   * player is using an
