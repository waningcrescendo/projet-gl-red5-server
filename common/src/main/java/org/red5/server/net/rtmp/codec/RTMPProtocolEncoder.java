...
public class RTMPProtocolEncoder implements Constants, IEventEncoder {
  ...
  /**
   * Tolerance (in milliseconds) for late media on streams. A set of levels based on this value will
   * be determined.
   */
  private long baseTolerance = 15000;

  /**
   * Middle tardiness level, between base and this value disposable frames will be dropped. Between
   * this and highest value regular interframes will be dropped.
   */
  ...
  /**
   * Indicates if we should drop live packets with future timestamp (i.e, when publisher bandwidth
  ...
  /** Represents the maximum size of a full RTMP chunk header */
  private static final int MAX_HEADER_SIZE = 18;

  /** The number of bytes skipped temporarily */
  private static final int SKIP_BYTES = 4;

  /** The size of the length field, used to store the length of a data segment */
  private static final int LENGTH_FIELD_SIZE = 4;

  /** The overhead size added to the total length */
  private static final int LENGTH_FIELD_OVERHEAD = 4;
  ...
  public IoBuffer encodePacket(Packet packet) {
    ...
    if (message instanceof ChunkSize) {
      ...
      ((RTMPConnection) Red5.getConnectionLocal())
          .getState()
          .setWriteChunkSize(chunkSizeMsg.getSize());
    }
    // normally the message is expected not to be dropped
    if (!dropMessage(channelId, message)) {
      // log.trace("Header time: {} message timestamp: {}", header.getTimer(),
      ...
      if (data != null) {
        ...
        int bufSize = dataLen + MAX_HEADER_SIZE + (numChunks * 2);
  ...
  /**
   * Determine if this message should be dropped. If the traffic from server to client is congested,
   * then drop LIVE messages to help alleviate congestion.
   *
   * <p>- determine latency between server and client using ping - ping timestamp is unsigned int (4
   * bytes) and is set from value on sender
   *
   * <p>1st drop disposable frames - lowest mark 2nd drop interframes - middle 3rd drop key frames -
   * high mark
   *
  ...
  protected boolean dropMessage(int channelId, IRTMPEvent message) {
    ...
    if (!dropEncoded) {
      log.trace(
          "Not dropping due to flag, source type: {} (0=vod,1=live)", message.getSourceType());
      return false;
    }
    ...
    if (isDroppable) {
      ...
      // Calculate when this message should have arrived. Take the time when the
      // stream started, add
      // the current message's timestamp and subtract the timestamp of the first
      // message.
      long clockTimeOfMessage =
          mapping.getClockStartTime() + timestamp - mapping.getStreamStartTime();

      // Determine how late this message is. This will tell us if the incoming path is
      // congested.
      long incomingLatency = clockTimeOfMessage - now;

      if (log.isDebugEnabled()) {
        log.debug(
            "incomingLatency={} clockTimeOfMessage={} getClockStartTime={} timestamp={} getStreamStartTime={} now={}",
            new Object[] {
              incomingLatency,
              clockTimeOfMessage,
              mapping.getClockStartTime(),
              timestamp,
              mapping.getStreamStartTime(),
              now
            });
      }
      ...
      if (isLiveStream && dropLiveFuture) {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "incomingLatency={} clockTimeOfMessage={} now={}",
              new Object[] {incomingLatency, clockTimeOfMessage, now});
        }
      }
      // NOTE: We could decide to drop the message here because it is very late due to
      // incoming
      // traffic congestion.

      // We need to calculate how long will this message reach the client. If the
      // traffic is
      // congested, we decide to drop the message.
      long outgoingLatency = 0L;
      if (conn != null) {
        // Determine the interval when the last ping was sent and when the pong was
        // received. The
        // duration will be the round-trip time (RTT) of the ping-pong message. If it is
        // high then
        // it
        ...
        if (lastPingPongInterval > 0) {
          ...
          if (log.isDebugEnabled()) {
            log.debug(
                "outgoingLatency={} lastPingTime={}",
                new Object[] {outgoingLatency, lastPingPongInterval});
          }
        }
      }
      ...
      if (log.isTraceEnabled()) {
        log.trace(
            "Packet timestamp: {}; latency: {}; now: {}; message clock time: {}, dropLiveFuture: {}",
      ...
      if (outgoingLatency < baseTolerance) {
        ...
        if (log.isDebugEnabled()) {
          log.debug(
              "Outgoing direction congested. outgoingLatency={} highestTolerance={}",
              new Object[] {outgoingLatency, highestTolerance});
        }
        if (isDroppable) {
  ...
  private byte getHeaderType(final Header header, final Header lastHeader) {
    // int lastFullTs = ((RTMPConnection)
    // Red5.getConnectionLocal()).getState().getLastFullTimestampWritten(header.getChannelId());
    if (lastHeader == null
        || header.getStreamId() != lastHeader.getStreamId()
        || header.getTimer() < lastHeader.getTimer()) {
  ...
  public void encodeHeader(Header header, Header lastHeader, IoBuffer buf) {
    ...
    if (log.isTraceEnabled()) {
      log.trace(
          "encodeHeader: {} lastHeader: {}", Header.HeaderType.values()[headerType], lastHeader);
    }
    /*
     * Timestamps in RTMP are given as an integer number of milliseconds relative to
     * an unspecified epoch. Typically, each stream will start with a timestamp of
     * 0, but this is not
     * required, as long as the two endpoints agree on the epoch. Note that this
     * means that any synchronization across multiple streams (especially from
     * separate hosts) requires some
     * additional mechanism outside of RTMP. Because timestamps are 32 bits long,
     * they roll over every 49 days, 17 hours, 2 minutes and 47.296 seconds. Because
     * streams are allowed to
     * run continuously, potentially for years on end, an RTMP application SHOULD
     * use serial number arithmetic [RFC1982] when processing timestamps, and SHOULD
     * be capable of handling
     * wraparound. For example, an application assumes that all adjacent timestamps
     * are within 2^31 - 1 milliseconds of each other, so 10000 comes after
     * 4000000000, and 3000000000
     * comes before 4000000000. Timestamp deltas are also specified as an unsigned
     * integer number of milliseconds, relative to the previous timestamp. Timestamp
     * deltas may be either 24
     * or 32 bits long.
     */
    ...
    // log.trace("Encoded chunk {} {}", Header.HeaderType.values()[headerType],
    // header);
  }
    ...
    for (final ISharedObjectEvent event : so.getEvents()) {
      ...
      switch (eventType) {
        ...
        case CLIENT_UPDATE_ATTRIBUTE:
          out.put(type);
          mark = out.position();
          out.skip(SKIP_BYTES); // we will be back
          output.putString(event.getKey());
          len = out.position() - mark - LENGTH_FIELD_SIZE;
          out.putInt(mark, len);
          break;
        ...
        case CLIENT_STATUS:
          ...
          out.putInt(message.length() + status.length() + LENGTH_FIELD_OVERHEAD);
