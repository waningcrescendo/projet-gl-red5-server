...
public class RTMPProtocolEncoder implements Constants, IEventEncoder {
  ...
  protected boolean dropMessage(int channelId, IRTMPEvent message) {
    ...
    if (isDroppable) {
      ...
      // NOTE: We could decide to drop the message here because it is very late due to incoming
      // traffic congestion.

      // We need to calculate how long will this message reach the client. If the traffic is
      ...
      if (conn != null) {
        ...
        // duration will be the round-trip time (RTT) of the ping-pong message. If it is high then
        // it
