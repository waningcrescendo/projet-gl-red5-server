...
public abstract class BasicScope implements IBasicScope, Comparable<BasicScope> {
  ...
  /**
   * Set to amount of time (in seconds) the scope will be kept before being freed, after the last
   * disconnect.
   */
  ...
  public boolean isValid() {
    ...
    return (type != null
        && !type.equals(ScopeType.UNDEFINED)
      ...
      if (removed && keepAliveJobName == null) {
        if (ScopeUtils.isRoom(this) && listeners.isEmpty()) {
          ...
          ISchedulingService schedulingService =
              (ISchedulingService) parent.getContext().getBean(ISchedulingService.BEAN_NAME);
          // by default keep a scope around for a fraction of a second
          keepAliveJobName =
              schedulingService.addScheduledOnceJob(
                  (keepDelay > 0 ? keepDelay * 1000 : 100), new KeepAliveJob(this));
        }
      }
    } else {
