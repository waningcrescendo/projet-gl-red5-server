...
public class AttributeStore implements ICastingAttributeStore {
  ...
  /**
   * Map for attributes with initialCapacity = 1, loadFactor = .9, concurrencyLevel = (# of
   * processors)
   */
  protected Map<String, Object> attributes =
      new ConcurrentHashMap<>(1, 0.5f, Runtime.getRuntime().availableProcessors());
  ...
  public boolean setAttribute(final String name, final Object value) {
    ...
    if (name != null && value != null) {
      ...
      // previous will be null if the attribute didn't exist and if it does it will equal the
      // previous value
      if (previous != null) {
        ...
        if (value instanceof Collection) {
          ...
          for (Object newCollectionEntry : newCollection) {
            ...
            // first element that does not exist in the previous collection will trigger the
            // modified result
            if (freq == 0) {
              result = true;
              break;
          ...
          // whether or not the new incoming value is "equal" to the previous value; not equal
          // equates to "modified"
          result = !value.equals(previous);
          if (log.isTraceEnabled()) {
            log.trace(
                "Equality check - modified: {} previous: {} new: {}", result, previous, value);
            log.trace("Class: {} {}", previous.getClass(), value.getClass());
          }
          // if the new value is not equal to the current / previous value and its a base-type or
          // array, replace it
