...
public class Server implements IServer, ApplicationContextAware, InitializingBean, DisposableBean {
  ...
  protected ConcurrentMap<String, IGlobalScope> globals =
      new ConcurrentHashMap<String, IGlobalScope>(1, 0.9f, 1);

  /** Mappings */
  protected ConcurrentMap<String, String> mapping =
      new ConcurrentHashMap<String, String>(32, 0.9f, 8);
  ...
  protected String getKey(String hostName, String contextPath) {
    return String.format(
        "%s/%s",
        (hostName == null ? EMPTY : hostName), (contextPath == null ? EMPTY : contextPath));
  }
  ...
  public boolean addMapping(String hostName, String contextPath, String globalName) {
    log.info(
        ...
        new Object[] {globalName, hostName, contextPath});
  ...
  public void notifyScopeCreated(IScope scope) {
    schedulingService.addScheduledOnceJob(
        notificationDelay, new ScheduledNotificationJob(JobAction.CREATED, scope));
  }
  ...
  public void notifyScopeRemoved(IScope scope) {
    schedulingService.addScheduledOnceJob(
        notificationDelay, new ScheduledNotificationJob(JobAction.REMOVED, scope));
  }

  /**
   * Notify listeners about an added basic scope.
   *
   * @param scope the scope that was added
   */
  public void notifyBasicScopeAdded(IBasicScope scope) {
    schedulingService.addScheduledOnceJob(
        notificationDelay, new ScheduledNotificationJob(JobAction.BASIC_ADD, scope));
  }
  ...
  public void notifyBasicScopeRemoved(IBasicScope scope) {
    schedulingService.addScheduledOnceJob(
        notificationDelay, new ScheduledNotificationJob(JobAction.BASIC_REMOVE, scope));
  }
  ...
  public void notifyConnected(IConnection conn) {
    schedulingService.addScheduledOnceJob(
        notificationDelay, new ScheduledNotificationJob(JobAction.CONNECTED, conn));
  }
  ...
  public void notifyDisconnected(final IConnection conn) {
    schedulingService.addScheduledOnceJob(
        notificationDelay, new ScheduledNotificationJob(JobAction.DISCONNECTED, conn));
  }
