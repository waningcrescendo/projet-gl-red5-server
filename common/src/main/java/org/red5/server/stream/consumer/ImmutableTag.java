...
/**
 * An ImmutableTag represents immutable encapsulation of flash media data. The timestamp is the only
 * mutable field.
 *
...
public class ImmutableTag implements ITag {
  ...
  public static ImmutableTag build(
      byte dataType, int timestamp, IoBuffer data, int previousTagSize) {
    if (data != null) {
      byte[] body = new byte[data.limit()];
      int pos = data.position();
      data.get(body);
