 ...
 * handling, and provides stream start and stop operations.
 *
...
public abstract class AbstractStream implements IStream {
  ...
  /**
   * Contains {@link PropertyChangeListener}s registered with this stream and following its changes
   * of state.
   */
  protected transient CopyOnWriteArraySet<PropertyChangeListener> stateListeners =
  ...
  /** Current state */
  protected final transient AtomicReference<StreamState> state =
      new AtomicReference<>(StreamState.UNINIT);

  /**
   * Creates a new {@link PropertyChangeEvent} and delivers it to all currently registered state
   * listeners.
   *
   * @param oldState the {@link StreamState} we had before the change
  ...
  protected void fireStateChange(StreamState oldState, StreamState newState) {
    final PropertyChangeEvent evt =
        new PropertyChangeEvent(this, "StreamState", oldState, newState);
