...
public class RecordingListener implements IRecordingListener {
    ...
    if (generator.resolvesToAbsolutePath()) {
      ...
      if (resource.exists()) {
        ...
        file =
            new File(
                String.format(
                    "%s/webapps/%s/%s", System.getProperty("red5.root"), appScopeName, fileName));
      }
    }
    return file;
  }
  ...
  public boolean init(IScope scope, String name, boolean isAppend) {
    ...
    if (file != null) {
      ...
      if (!isAppend) {
        if (file.exists()) {
          // when "live" or "record" is used, any previously recorded stream with the same stream
          // URI is deleted.
          if (!file.delete()) {
        ...
        if (file.exists()) {
          ...
          // if a recorded stream at the same URI does not already exist, "append" creates the
          // stream as though "record" was passed.
          isAppend = false;
        }
      }
      ...
      if (scope.getContext().hasBean("keyframe.cache")) {
        IKeyFrameMetaCache keyFrameCache =
            (IKeyFrameMetaCache) scope.getContext().getBean("keyframe.cache");
  ...
  public void packetReceived(IBroadcastStream stream, IStreamPacket packet) {
    if (recording.get()) {
      ...
      log.info(
          "A packet was received by recording listener, but it's not recording anymore. {}",
