...
public class StreamService implements IStreamService {
  ...
  public void initStream(Number streamId) {
    IConnection conn = Red5.getConnectionLocal();
    log.info(
        "initStream stream id: {} current stream id: {} connection: {}",
        streamId,
        conn.getStreamId(),
        conn.getSessionId());
    if (conn instanceof IStreamCapableConnection) {
  ...
  public void play(String name, int start, int length, boolean flushPlaylist) {
    ...
    if (conn instanceof IStreamCapableConnection) {
      ...
      if (stream == null) {
        if (log.isTraceEnabled()) {
          log.trace(
              "Stream not found for stream id: {} streams: {}",
              streamId,
              streamConn.getStreamsMap());
        }
        try {
          ...
          sendNSFailed(
              streamConn, StatusCodes.NS_FAILED, "Unable to start playing stream", name, streamId);
          return;
        }
      }
  ...
  public void publish(String name, String mode) {
    ...
    if (name != null && name.contains("?")) {
      ...
      name = name.substring(0, name.indexOf("?"));
      log.debug(
          "publish called with name: {} and mode: {}; query string: {}", name, mode, queryString);
    } else {
      ...
      if (stream != null) {
        ...
        if (nameAliasingEnabled && ((IClientBroadcastStream) stream).aliasRegistered(name)) {
          sendNSFailed(
              streamConn,
              StatusCodes.NS_PUBLISH_BADNAME,
              "You are not allowed to publish the stream, alias exists.",
              name,
              streamId);
  ...
  private void sendNSFailed(
      IConnection conn, String errorCode, String description, String name, Number streamId) {
  ...
  private void sendNSStatus(
      IConnection conn, String statusCode, String description, String name, Number streamId) {
   ...
   */
  public static void sendNetStreamStatus(
      IConnection conn,
      String statusCode,
      String description,
      String name,
    ...
    if (conn instanceof RTMPConnection) {
      ...
      Optional.ofNullable(rtmpConn.getChannel(rtmpConn.getChannelIdForStreamId(streamId)))
          .ifPresent(c -> c.sendStatus(s));
    } else {
