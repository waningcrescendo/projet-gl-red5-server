...
public class PlaylistSubscriberStream extends AbstractClientStream
  ...
  /**
   * Creates a play engine based on current services (scheduling service, consumer service, and
   * provider service). This method is useful during unit testing.
   */
  PlayEngine createEngine(
    ...
    engine =
        new PlayEngine.Builder(this, schedulingService, consumerService, providerService).build();
    ...
    if (engine == null) {
      ...
      if (scope != null) {
        ...
        if (ctx.hasBean(ISchedulingService.BEAN_NAME)) {
          ...
          schedulingService =
              (ISchedulingService)
                  scope.getParent().getContext().getBean(ISchedulingService.BEAN_NAME);
        }
        ...
        if (ctx.hasBean(IConsumerService.KEY)) {
          ...
          // try the parent
          consumerService =
              (IConsumerService) scope.getParent().getContext().getBean(IConsumerService.KEY);
        }
        ...
        if (ctx.hasBean(IProviderService.BEAN_NAME)) {
          ...
          // try the parent
          providerService =
              (IProviderService) scope.getParent().getContext().getBean(IProviderService.BEAN_NAME);
        }
        engine =
            new PlayEngine.Builder(this, schedulingService, consumerService, providerService)
                .build();
      } else {
  ...
  public void onChange(final StreamState state, final Object... changed) {
    ...
    switch (state) {
      case SEEK:
        ...
        if (handler != null) {
          notifier =
              new Notifier(this, handler, conn) {
  ...
  public class Notifier implements IScheduledJob {
    ...
    public Notifier(
        IPlaylistSubscriberStream stream, IStreamAwareScopeHandler handler, IConnection conn) {
