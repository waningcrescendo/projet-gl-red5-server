 ...
 *
 * <p>Note that recorded streams are recorded as FLV files.
 *
 ...
 */
@ManagedResource(
    objectName = "org.red5.server:type=ClientBroadcastStream",
...
public class ClientBroadcastStream extends AbstractClientStream
  ...
  protected void notifyBroadcastStart() {
    ...
    params.put(
        "creationdate",
        ZonedDateTime.ofInstant(cal.toInstant(), ZoneId.of("UTC"))
  ...
  public void saveAs(String name, boolean isAppend) throws IOException {
    ...
    if (recordingListener == null) {
      ...
      // IRecordingListener listener = (IRecordingListener)
      // ScopeUtils.getScopeService(conn.getScope(), IRecordingListener.class,
      // RecordingListener.class, false);
      ...
      log.debug(
          "Recording listener already exists for stream: {} auto record enabled: {}",
          name,
          automaticRecording);
    }
  }
        ...
        if (scope.hasHandler()) {
          ...
          if (handler instanceof IStreamAwareScopeHandler) {
            if (recordingListener != null && recordingListener.get().isRecording()) {
              ...
              //                                    if (!file.delete()) {
              //                                        log.debug("File was not deleted: {}",
              // file.getAbsoluteFile());
              //                                    }
              //                                }
              //                            } catch (Exception e) {
              //                                log.warn("Exception removing previously recorded
              // file", e);
              //                            }
    ...
    if (registerJMX && StringUtils.isNotEmpty(publishedName) && !"false".equals(publishedName)) {
      ...
      try {
        ...
        ObjectName oName =
            new ObjectName(
                String.format(
                    "org.red5.server:type=ClientBroadcastStream,scope=%s,publishedName=%s",
                    getScope().getName(), publishedName.replaceAll(":", "|")));
