 ...
 *
 * <p>Add this java option -Dlogback.ContextSelector=org.red5.logging.LoggingContextSelector
 *
...
public class LoggingContextSelector implements ContextSelector {
  ...
  public LoggingContextSelector(LoggerContext context) {
    if (DEFAULT_CONTEXT == null) {
      ...
      // inject the name of the current application as "application-name" property of the
      // LoggerContext
  ...
  public LoggerContext getLoggerContext(String contextName) {
    if (Red5LoggerFactory.DEBUG) {
      System.out.printf(
          "getLoggerContext request for %s in context map %s%n",
          contextName, contextMap.containsKey(contextName));
    }
  ...
  public LoggerContext getLoggerContext(String contextName, URL url) {
    if (Red5LoggerFactory.DEBUG) {
      System.out.printf(
          "getLoggerContext request for %s in context map %s url: %s%n",
          contextName, contextMap.containsKey(contextName), url);
    }
    LoggerContext context = contextMap.get(contextName);
    if (context == null) {
      try {
  ...
  public void attachLoggerContext(String contextName, LoggerContext loggerContext) {
    if (Red5LoggerFactory.DEBUG) {
      System.out.printf(
          "Adding logger context: %s to map for context: %s%n",
   ...
   *
   * <p>They provide a way to tell the selector which context to use, thus saving the cost of a JNDI
   * call at each new request.
   *
