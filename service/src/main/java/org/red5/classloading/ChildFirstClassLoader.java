...
/**
 * An almost trivial no-fuss implementation of a class loader following the child-first delegation
 * model. <i>Based on code from Ceki Gulcu</i>
 *
...
public final class ChildFirstClassLoader extends URLClassLoader {
  ...
  private void dumpClassLoaderNames() {
    System.out.printf(
        "[ChildFirstClassLoader] Classloaders:\nSystem %s\nParents Parent %s\nParent %s\nThis class %s\nTCL %s\n\n",
        system,
        parentParent,
        this.parent,
   ...
   *
   * <p>The implementation is surprisingly straightforward.
   *
  ...
  /**
   * Override the parent-first resource loading model established by java.lang.Classloader with
   * child-first behavior.
   *
