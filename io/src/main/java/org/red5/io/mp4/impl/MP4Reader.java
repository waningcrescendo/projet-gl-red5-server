 ...
 *
 * <ul>
 *   <li>NetStream.Play.FileStructureInvalid: This event is sent if the player detects an MP4 with
 *       an invalid file structure. Flash Player cannot play files that have invalid file
 *       structures.
 *   <li>NetStream.Play.NoSupportedTrackFound: This event is sent if the player does not detect any
 *       supported tracks. If there aren't any supported video, audio or data tracks found, Flash
 *       Player does not play the file.
 * </ul>
 *
...
public class MP4Reader implements IoConstants, ITagReader, IKeyFrameDataAnalyzer {
  ...
  public static final byte[] PREFIX_AUDIO_CONFIG_FRAME = new byte[] {(byte) 0xaf, (byte) 0};
  ...
  public static final byte[] EMPTY_AAC = {
    (byte) 0x21, (byte) 0x10, (byte) 0x04, (byte) 0x60, (byte) 0x8c, (byte) 0x1c
  };

  /** Video packet prefix for the decoder frame */
  public static final byte[] PREFIX_VIDEO_CONFIG_FRAME =
      new byte[] {(byte) 0x17, (byte) 0, (byte) 0, (byte) 0, (byte) 0};
  ...
  /**
   * Container for seek points in the video. These are the time stamps for the key frames or
   * samples.
   */
  ...
  /**
   * This handles the moov atom being at the beginning or end of the file, so the mdat may also be
   * before or after the moov atom.
   */
                                ...
                                if (mdbox instanceof MediaHeaderBox) {
                                  ...
                                  if (log.isDebugEnabled()) {
                                    log.debug(
                                        "hdlr: {} {} {}",
                                        hdlr.getFourcc(),
                                        hdlr.getComponentType(),
                                        hdlr.getComponentSubType());
                                  }
                                    ...
                                    switch (sbox.getFourcc()) {
                                      case "stsd":
                                        SampleDescriptionBox stsd =
                                            (SampleDescriptionBox) sbox; // stsd
                                        // stsd: mp4a, avc1, mp4v
                                        stsd.getBoxes()
                                            .forEach(
                                                stbox -> {
                                                  ...
                                                  switch (stbox.getFourcc()) {
                                                    case "mp4a":
                                                      audioCodecId = "mp4a";
                                                      processAudioSampleEntry(
                                                          (AudioSampleEntry) stbox, scale.get());
                                                      break;
                                                    case "ac-3":
                                                      audioCodecId = "ac-3";
                                                      processAudioSampleEntry(
                                                    ...
                                                    case "mp4v":
                                                      videoCodecId = "mp4v";
                                                      processVideoSampleEntry(
                                                          (VideoSampleEntry) stbox, scale.get());
                                                      break;
                                                    case "avc1":
                                                      videoCodecId = "avc1";
                                                      // AVC1Box avc1 = Box.asBox(AVC1Box.class,
                                                      // stbox);
                                                      processVideoSampleEntry(
                                                          (VideoSampleEntry) stbox, scale.get());
                                                      break;
                                                    case "hev1":
                                                      videoCodecId = "hev1";
                                                      // HEV1Box hev1 = Box.asBox(HEV1Box.class,
                                                      // stbox);
                                                      // processVideoSampleEntry((VideoSampleEntry)
                                                      // stbox, scale.get());
                                                      break;
                                                    case "hvc1":
                                                      videoCodecId = "hvc1";
                                                      // HVC1Box hvc1 = Box.asBox(HVC1Box.class,
                                                      // stbox);
                                                      processVideoSampleEntry(
                                                          (VideoSampleEntry) stbox, scale.get());
                                                      break;
                                                    default:
                                                      log.warn(
                                                          "Unhandled sample description box: {}",
                                                          stbox);
                                                      break;
                                                  }
                                                });
                                      ...
                                      case "stsc": // records
                                        ...
                                        if (isAudio.get()) {
                                          ...
                                          if (log.isDebugEnabled()) {
                                            log.debug(
                                                "Audio samples to chunks: {}", ascEntries.length);
                                            for (SampleToChunkEntry entry : ascEntries) {
                                              log.debug(
                                                  "Audio s2c count: {} first: {} entry: {}",
                                                  entry.getCount(),
                                                  entry.getFirst(),
                                                  entry.getEntry());
                                            }
                                          ...
                                          // handle instance where there are no actual records (bad
                                          // f4v?)
                                        } else if (isVideo.get()) {
                                          SampleToChunkEntry[] vscEntries = stsc.getSampleToChunk();
                                          if (log.isDebugEnabled()) {
                                            log.debug(
                                                "Video samples to chunks: {}", vscEntries.length);
                                            for (SampleToChunkEntry entry : vscEntries) {
                                              log.debug(
                                                  "Video s2c count: {} first: {} entry: {}",
                                                  entry.getCount(),
                                                  entry.getFirst(),
                                                  entry.getEntry());
                                            }
                                      ...
                                      case "co64":
                                        ...
                                        if (isAudio.get()) {
                                          ...
                                          log.debug("Chunk count: {}", videoChunkOffsets.length);
                                          // double the timescale for video, since it seems to run
                                          // at
                                      ...
                                      case "stts":
                                        ...
                                        if (records.length > 0) {
                                          ...
                                          log.debug(
                                              "Samples: {} duration: {} segment duration: {}",
                                              rec.getSampleCount(),
                                              rec.getSampleDuration(),
                                              rec.getSegmentDuration());
                                        }
                                        if (isAudio.get()) {
                                          log.debug("Audio time to samples: {}", records.length);
                                          // if we have 1 record it means all samples have the same
                                          // duration
                                      ...
                                      case "ctts": // ctts - (composition) time to sample
                                        ...
                                        if (log.isTraceEnabled()) {
                                          for (CompositionOffsetsBox.Entry rec : compositionTimes) {
                                            ...
                                            if (scale.get() > 0) {
                                              rec.offset +=
                                                  (offset / (double) scale.get()) * 1000.0;
                                            }
                                            log.trace(
                                                "Samples = {} offset = {}",
                                                rec.getCount(),
            ...
            box -> {
              ...
              switch (box.getFourcc()) {
                case "esds":
                  ...
                  log.debug(
                      "Process {} obj: {} avg bitrate: {} max bitrate: {}",
                      esds.getFourcc(),
                      esds.getObjectType(),
                      esds.getAvgBitrate(),
                      esds.getMaxBitrate());
                  ...
                  log.debug(
                      "Audio config bytes: {}", HexDump.byteArrayToHexString(audioDecoderBytes));
                  // the first 5 (0-4) bits tell us about the coder used for aacaot/aottype
                  // http://wiki.multimedia.cx/index.php?title=MPEG-4_Audio 0 - NULL 1 - AAC Main (a
                  // deprecated AAC profile
                  // from MPEG-2) 2 - AAC LC or backwards compatible HE-AAC 3 - AAC Scalable Sample
                  // Rate 4 - AAC LTP (a replacement for AAC Main, rarely used) 5 - HE-AAC
                  // explicitly signaled
                  // (Non-backward compatible) 23 - Low Delay AAC 29 - HE-AACv2 explicitly signaled
                  // 32 - MP3on4 Layer 1 33 - MP3on4 Layer 2 34 - MP3on4 Layer 3
                  //
                  ...
                  log.debug(
                      "Audio coder type: {} {} id: {}",
                      audioCoderType,
                      Integer.toBinaryString(audioCoderType),
                      audioCodecId);
                  break;
                ...
                case "wave":
                  ...
                  // wave/esds
                  EsdsBox wesds =
                      wave.getBoxes().stream()
                          .filter(b -> b instanceof EsdsBox)
                          .map(b -> (EsdsBox) b)
                          .findFirst()
                          .orElse(null);
                  if (wesds != null) {
            ...
            box -> {
              ...
              switch (box.getFourcc()) {
                case "esds": // videoCodecId = "mp4v"
                  ...
                  log.debug(
                      "Video config bytes: {}", HexDump.byteArrayToHexString(videoDecoderBytes));
                  break;
                case "avcC": // videoCodecId = "avc1"
                  ...
                  log.debug(
                      "Process {} level: {} nal len: {} profile: {} compat: {}",
                      avcC.getFourcc(),
                      avcLevel,
                      avcC.getNalLengthSize(),
                      avcProfile,
                      avcC.getProfileCompat());
                ...
                case "colr": // color
                  ...
                  log.debug(
                      "Process {} primaries: {} transfer: {} matrix: {}",
                      colr.getFourcc(),
   ...
   *
   * <p>Packet prefixes: 17 00 00 00 00 = Video extra data (first video packet) 17 01 00 00 00 =
   * Video keyframe 27 01 00 00 00 = Video interframe af 00 ... 06 = Audio extra data (first audio
   * packet) af 01 = Audio frame
   *
   * <p>Audio extra data(s): af 00 = Prefix 11 90 4f 14 = AAC Main = aottype 0 // 11 90 12 10 = AAC
   * LC = aottype 1 13 90 56 e5 a5 48 00 = HE-AAC SBR = aottype 2 06 = Suffix
   *
   * <p>Still not absolutely certain about this order or the bytes - need to verify later
   */
  ...
  public ITag readTag() {
    ...
    if (log.isTraceEnabled()) {
      log.trace(
          "Read tag - prevFrameSize {} audio: {} video: {}",
          new Object[] {prevFrameSize, audioCount, videoCount});
    }
    ...
    if (!frames.isEmpty()) {
      try {
        ...
        if (frame != null) {
          ...
          log.debug(
              "Read tag - type: {} body size: {}",
              (type == TYPE_AUDIO ? "Audio" : "Video"),
              tag.getBodySize());
  ...
  /**
   * Performs frame analysis and generates metadata for use in seeking. All the frames are analyzed
   * and sorted together based on time and offset.
   */
            ...
            if (audioSamples.length > 0) {
              ...
              if (size == 6) {
                try {
                  ...
                  log.trace(
                      "Audio bytes: {} equal: {}",
                      HexDump.byteArrayToHexString(tmp),
                      Arrays.equals(EMPTY_AAC, tmp));
