...
public class FLVReader implements IoConstants, ITagReader, IKeyFrameDataAnalyzer {
  ...
  private void postInitialize() {
    if (log.isDebugEnabled()) {
      log.debug(
          "FLVReader 1 - Buffer size: {} position: {} remaining: {}",
          new Object[] {getTotalBytes(), getCurrentPosition(), getRemainingBytes()});
    }
  ...
  public KeyFrameMeta analyzeKeyFrames() {
    ...
    try {
      ...
      while (hasMoreTags()) {
        ...
        // XXX Paul: this 'properly' handles damaged FLV files - as far as duration/size is
        // concerned
  ...
  private ITag readTagHeader() throws UnsupportedDataTypeException {
    ...
    switch (dataType) {
      ...
      default:
        log.debug(
            "Invalid data type detected ({}), reading ahead\n current position: {} limit: {}",
            dataType,
            in.position(),
            in.limit());
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Data type: {} timestamp: {} stream id: {} body size: {} previous tag size: {}",
          new Object[] {dataType, timestamp, streamId, bodySize, previousTagSize});
    }
