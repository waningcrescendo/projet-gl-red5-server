 ...
 *
 * <p>factory for creating matroska tags, it use property file -
 * matroska_type_definition_config.properties with structure: long id = "name provided
 * specification","java class representing tag data"
 */
public class TagFactory {
  ...
  static {
    ...
    try (InputStream input =
        TagFactory.class.getResourceAsStream("matroska_type_by_id_definition.properties")) {
      props.load(input);
  ...
  public static Tag createTag(String tagName) throws ConverterException {
    ...
    try {
      Tag newTag =
          (Tag) it.clazz.getConstructor(String.class, VINT.class).newInstance(tagName, typeVint);
      return newTag;
    } catch (Exception e) {
  ...
  private static class NameTag {
    ...
    private NameTag(Object prop) throws ClassNotFoundException {
      ...
      this.clazz =
          (Class<? extends Tag>)
              Class.forName(TagFactory.class.getPackage().getName() + "." + parameters[1]);
    }
  }
