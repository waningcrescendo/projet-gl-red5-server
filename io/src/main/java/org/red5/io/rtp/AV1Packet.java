...
public class AV1Packet {
  ...
  private static final byte zMask = (byte) 0b10000000;

  private static final byte zBitshift = (byte) 7;

  private static final byte yMask = (byte) 0b01000000;

  private static final byte yBitshift = (byte) 6;

  private static final byte wMask = (byte) 0b00110000;

  private static final byte wBitshift = (byte) 4;

  private static final byte nMask = (byte) 0b00001000;

  private static final byte nBitshift = (byte) 3;
  ...
  // N: MUST be set to 1 if the packet is the first packet of a coded video sequence, and MUST be
  // set to 0 otherwise.
  public boolean N;

  // Each AV1 RTP Packet is a collection of OBU Elements. Each OBU Element may be a full OBU, or
  // just a fragment of one.
    ...
    if (Math.min(maxFragmentSize, payloadDataRemaining) > 0) {
      while (payloadDataRemaining > 0) {
        ...
        System.arraycopy(
            ...
            payloadDataIndex,
            out,
            av1PayloaderHeadersize + leb128Size,
            currentFragmentSize);
        payloads.add(out);
  ...
  // Unmarshal parses the passed byte slice and stores the result in the AV1Packet this method is
  // called upon
  public boolean unmarshal(byte[] payload) {
    ...
    if (payload != null && payload.length > 1) {
      ...
      if (!Z && !N) {
        ...
        bytesRead = 2;
        System.out.printf(
            "Index: %d and obu element length: %d bytes read: %d%n",
            currentIndex, obuElementLength, bytesRead);
        for (int i = 1; currentIndex < payload.length; i++) {
          ...
          OBUElements.add(
              Arrays.copyOfRange(payload, currentIndex, currentIndex + obuElementLength));
  ...
  public static void main(String[] args) {
    ...
    byte[][] out2 =
        new byte[][] {
          {0x40, 0x02, 0x00, 0x01},
          {(byte) 0xc0, 0x02, 0x02, 0x03},
          {(byte) 0xc0, 0x02, 0x04, 0x04},
          {(byte) 0x80, 0x01, 0x05}
        };
