...
public final class UnsignedLong extends UnsignedNumber {
  ...
  public static UnsignedLong fromBytes(byte[] c, int offset) {
    ...
    for (int i = 7; i >= 0; i--) number.value[i] = c[offset + i];
    return number;
  }
  ...
  public static UnsignedLong fromString(String c, int radix) {
    ...
    for (int i = 0; i < len; i++) number.value[7 - i] = bytes[bytes.length - 1 - i];
    return number;
  }
  ...
  public long longValue() {
    return (((long) value[0] << 56) & 0xFF00000000000000L
        ...
        | ((long) value[5] << 16) & 0xFF0000L
        | ((long) value[6] << 8) & 0xFF00L
        | ((value[7]) & 0xFFL));
  }
  ...
  public int compareTo(UnsignedNumber other) {
    ...
    for (int i = 0; i < 8; i++)
      if ((char) value[i] > (char) normalValue[i]) return +1;
      else if ((char) value[i] < (char) normalValue[i]) return -1;
    return 0;
  }
  ...
  public boolean equals(Object other) {
    if (other instanceof UnsignedLong) {
      ...
      for (int i = 7; i >= 0; i--) if (value[i] != bytes[i]) return false;
      return true;
    } else if (other instanceof Number) return longValue() == ((Number) other).longValue();
    else return false;
  }
  ...
  public void shiftRight(int nBits) {
    ...
    if (nBits % 8 != 0) throw new IllegalArgumentException("nBits must be a multiple of 8.");

    int nBytes = nBits / 8;
    for (int i = 7; i >= nBytes; i--) value[i] = value[i - nBytes];
    for (int i = nBytes - 1; i >= 0; i--) value[i] = 0;
  }
