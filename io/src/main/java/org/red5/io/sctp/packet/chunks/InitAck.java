...
public final class InitAck extends Chunk {
  ...
  public InitAck(final byte[] data, int offset, int length) throws SctpException {
    ...
    stateCookie =
        new StateCookie(
            data,
            offset + MANDATORY_FIELD_SIZE + CHUNK_HEADER_SIZE,
            length - (MANDATORY_FIELD_SIZE + CHUNK_HEADER_SIZE));
  }

  public InitAck(int initiateTag, int initialTSN, StateCookie stateCookie, Mac mac)
    ...
    this.advertisedReceiverWindowCredit =
        IAssociationControl.DEFAULT_ADVERTISE_RECEIVE_WINDOW_CREDIT;
  ...
  public byte[] getBytes() {
    ByteBuffer byteBuffer =
        ByteBuffer.allocateDirect(MANDATORY_FIELD_SIZE + super.getSize() + stateCookieBytes.length);
  ...
  public void apply(IAssociationControl channel)
      throws IOException, InvalidKeyException, NoSuchAlgorithmException, SctpException {
    ...
    SctpPacket packet =
        new SctpPacket(
            (short) channel.getSourcePort(), (short) channel.getDestinationPort(), 0, cookieEcho);
