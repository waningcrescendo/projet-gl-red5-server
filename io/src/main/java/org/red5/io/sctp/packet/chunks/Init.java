...
public class Init extends Chunk {
  ...
  // initiateTag(4 byte) + advertisedReceiverWindowCredit(4 byte) + numberOfOutboundStreams(2 byte)
  // + numberOfInboundStreams(2 byte) + TSN(4 byte)
  ...
  public Init(final byte[] data, int offset, int length) throws SctpException {
    ...
    ByteBuffer byteBuffer =
        ByteBuffer.wrap(
            data, offset + CHUNK_HEADER_SIZE, data.length - (offset + CHUNK_HEADER_SIZE));
  ...
  public void apply(InetSocketAddress address, IServerChannelControl server)
    ...
    SctpPacket packet =
        new SctpPacket(server.getPort(), address.getPort(), getInitiateTag(), initAck);
