...
public class InboundHandshake extends RTMPHandshake {
  ...
  public IoBuffer decodeClientRequest1(IoBuffer in) {
    ...
    if (useEncryption()) {
      ...
      //    handshakeType = 8;
      // }
      // get the DH offset in the handshake bytes, generates DH keypair, and adds the public key to
      // handshake bytes
    ...
    log.debug(
        "Server digest: {}",
        Hex.encodeHexString(
            Arrays.copyOfRange(s1, digestPosServer, digestPosServer + DIGEST_LENGTH)));
    ...
    log.debug(
        "Client digest: {}",
        Hex.encodeHexString(
            Arrays.copyOfRange(c1, digestPosClient, digestPosClient + DIGEST_LENGTH)));
    // swfVerification bytes are the sha256 hmac hash of the decompressed swf, the key is the last
    // 32 bytes of the server handshake
    if (swfSize > 0) {
    ...
    calculateHMAC_SHA256(
        c1, digestPosClient, DIGEST_LENGTH, GENUINE_FMS_KEY, GENUINE_FMS_KEY.length, digestResp, 0);
    log.debug("Digest response (key): {}", Hex.encodeHexString(digestResp));
    calculateHMAC_SHA256(
        c1,
        0,
        (Constants.HANDSHAKE_SIZE - DIGEST_LENGTH),
        digestResp,
        DIGEST_LENGTH,
        signatureResponse,
        0);
    ...
    System.arraycopy(
        signatureResponse, 0, c1, (Constants.HANDSHAKE_SIZE - DIGEST_LENGTH), DIGEST_LENGTH);
      ...
      log.debug(
          ...
          Hex.encodeHexString(
              Arrays.copyOfRange(
                  c2,
                  (Constants.HANDSHAKE_SIZE - DIGEST_LENGTH),
                  (Constants.HANDSHAKE_SIZE - DIGEST_LENGTH) + DIGEST_LENGTH)));
      // verify client response
      calculateHMAC_SHA256(
          s1, digestPosServer, DIGEST_LENGTH, GENUINE_FP_KEY, GENUINE_FP_KEY.length, digest, 0);
      calculateHMAC_SHA256(
          c2, 0, Constants.HANDSHAKE_SIZE - DIGEST_LENGTH, digest, DIGEST_LENGTH, signature, 0);
      if (useEncryption()) {
        switch (handshakeType) {
          case RTMPConnection.RTMP_ENCRYPTED:
            log.debug("RTMPE type 6");
        ...
        // update 'encoder / decoder state' for the RC4 keys both parties *pretend* as if handshake
        // part 2 (1536 bytes) was encrypted
        // effectively this hides / discards the first few bytes of encrypted session which is known
        // to increase the secure-ness of RC4
        // RC4 state is just a function of number of bytes processed so far that's why we just run
        // 1536 arbitrary bytes through the keys below
  ...
  private boolean validateScheme(byte[] handshake, int scheme) {
    ...
    System.arraycopy(
        handshake,
        digestOffset + DIGEST_LENGTH,
        tempBuffer,
        digestOffset,
        Constants.HANDSHAKE_SIZE - digestOffset - DIGEST_LENGTH);
