...
public class RTMPTServlet extends HttpServlet {
   ...
   * @throws IOException I/O exception
   */
  protected void returnMessage(RTMPTConnection conn, IoBuffer buffer, HttpServletResponse resp)
      throws IOException {
  ...
  /**
   * Sets the request info for the current request. Request info contains the session id and request
   * number gathered from the incoming request. The URI is in this form /[method]/[session
   * id]/[request number] ie. /send/CAFEBEEF01/7
   *
  ...
  protected void returnPendingMessages(RTMPTConnection conn, HttpServletResponse resp) {
    ...
    if (data != null) {
      try {
        ...
        // this can be an IOException or a container specific one like ClientAbortException from
        // catalina
  ...
  protected void handleClose(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    if (connection != null) {
      ...
      handleBadRequest(
          String.format("Close: unknown client session: %s", requestInfo.get().getSessionId()),
          resp);
    }
  }
  ...
  protected void handleSend(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    if (conn != null) {
      ...
      handleBadRequest(
          String.format("Send: unknown client session: %s", requestInfo.get().getSessionId()),
          resp);
    }
  }
  ...
  protected void handleIdle(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    if (conn != null) {
      ...
      handleBadRequest(
          String.format("Idle: unknown client session: %s", requestInfo.get().getSessionId()),
          resp);
    }
  }
  ...
  protected void service(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    ...
    log.debug(
        "Request - method: {} content type: {} path: {}",
    ...
    if (enforceContentTypeCheck && !CONTENT_TYPE.equals(req.getContentType())) {
      handleBadRequest(
          String.format("Bad request, unsupported content type: %s.", req.getContentType()), resp);
      return;
    }
    // get the uri
    ...
    // since the only current difference in the type of request that we are interested in is the
    // 'second' character, we can double
  ...
  public void destroy() {
    // Cleanup connections
    List<BaseConnection> conns =
        manager.getAllConnections().stream()
            .filter(c -> (c instanceof RTMPTConnection))
