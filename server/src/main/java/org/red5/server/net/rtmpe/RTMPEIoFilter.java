...
public class RTMPEIoFilter extends IoFilterAdapter {
  ...
  public void messageReceived(NextFilter nextFilter, IoSession session, Object obj)
    ...
    if (sessionId != null) {
      ...
      InboundHandshake handshake =
          session.containsAttribute(RTMPConnection.RTMP_HANDSHAKE)
              ? (InboundHandshake) session.getAttribute(RTMPConnection.RTMP_HANDSHAKE)
              : null;
          ...
          if (c2Size >= Constants.HANDSHAKE_SIZE) {
            ...
            if (handshake != null) {
              if (handshake.decodeClientRequest2(IoBuffer.wrap(dst))) {
                ...
                // leave the remaining bytes in the buffer for the next step to handle / decrypt /
                // decode
              } else {
                log.warn("Client was rejected due to invalid handshake");
                conn.close();
          ...
          if (buffer.getBufferSize() > 0) {
            ...
            if (!((RTMPConnection) conn).isEncrypted()) {
              if (isTrace) {
                ...
                log.trace(
                    sessionId + " Receiving message: {}",
                    Hex.toHexString(message.array(), message.arrayOffset(), message.remaining()));
              }
