 ...
 *
 * <p>Transport Layer Security (TLS) Renegotiation Issue
 * http://www.oracle.com/technetwork/java/javase/documentation/tlsreadme2-176330.html Secure
 * renegotiation
 * https://jce.iaik.tugraz.at/sic/Products/Communication-Messaging-Security/iSaSiLk/documentation/Secure-Renegotiation
 * Troubleshooting a HTTPS TLSv1 handshake
 * http://integr8consulting.blogspot.com/2012/02/troubleshooting-https-tlsv1-handshake.html How to
 * analyze Java SSL errors http://www.smartjava.org/content/how-analyze-java-ssl-errors
 *
...
public class RTMPSMinaIoHandler extends RTMPMinaIoHandler {
  ...
  /**
   * Indicates that we would like to authenticate the client but if client certificates are
   * self-signed or have no certificate chain then we are still good
   */
  ...
  public void sessionCreated(IoSession session) throws Exception {
    ...
    try {
      sslContext =
          TLSFactory.getTLSContext(
              keyStoreType, keystorePassword, keystorePath, truststorePassword, truststorePath);
      ...
      if (log.isDebugEnabled()) {
        Arrays.asList(params.getCipherSuites())
            .forEach(cipher -> log.debug("Supported cipher suite: {}", cipher));
      }
      ...
      if (log.isDebugEnabled()) {
        log.debug(
            "SSL context params - need client auth: {} want client auth: {} endpoint id algorithm: {}",
