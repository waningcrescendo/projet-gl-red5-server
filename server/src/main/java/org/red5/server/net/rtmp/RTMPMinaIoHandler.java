...
public class RTMPMinaIoHandler extends IoHandlerAdapter {
  ...
  /**
   * Cache isTrace to avoid calling into logback on packet thread during production. Good for stream
   * threads, but no real benefit during setups/teardowns. Also helps during remote debugging to
   * activate/deactivate in real time.
   */
  private static final boolean isTrace = log.isTraceEnabled();

  /**
   * Cache isDebug to avoid calling into logback on packet thread during production. Good for stream
   * threads, but no real benefit during setups/teardowns. Also helps during remote debugging to
   * activate/deactivate in real time.
   */
  ...
  public void sessionOpened(IoSession session) throws Exception {
    ...
    session.setAttribute(
        ...
        new WeakReference<IConnectionManager<BaseConnection>>(connManager));
  ...
  public void setCodecFactory(ProtocolCodecFactory codecFactory) {
    log.warn(
        "This option is deprecated, the codec factory is now contained within the RTMPEIoFilter");
  }
