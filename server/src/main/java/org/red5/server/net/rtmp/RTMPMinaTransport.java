...
/**
 * Transport setup class configures socket acceptor and thread pools for RTMP in Mina. <br>
...
public class RTMPMinaTransport implements RTMPMinaTransportMXBean {
  ...
  public void start() throws Exception {
    ...
    if (enableDefaultAcceptor) {
      ...
      executor =
          new ThreadPoolExecutor(
              ...
              new LinkedBlockingQueue<Runnable>(Short.MAX_VALUE));
    ...
    // sets the interval (seconds) between each throughput calculation, the default value is 3
    // seconds
    ...
    if (trafficClass == -1) {
      ...
      // http://docs.oracle.com/javase/6/docs/api/java/net/Socket.html#setTrafficClass(int)
      // IPTOS_LOWCOST (0x02)
    ...
    // get info
    log.info(
        "Send buffer size: {} recv buffer size: {} so linger: {} traffic class: {}",
        new Object[] {
          sessionConf.getSendBufferSize(),
          sessionConf.getReceiveBufferSize(),
          sessionConf.getSoLinger(),
          sessionConf.getTrafficClass()
        });
    ...
    try {
      ...
      if (enableMinaMonitor) {
        ...
        try {
          ...
          mbs.registerMBean(
              new StandardMBean(this, RTMPMinaTransportMXBean.class, true),
  ...
  public String getStatistics() {
    ...
    if (stats != null) {
      // returns the cumulative number of sessions which were managed (or are being managed) by this
      // service, which means 'currently managed session count + closed session count'
