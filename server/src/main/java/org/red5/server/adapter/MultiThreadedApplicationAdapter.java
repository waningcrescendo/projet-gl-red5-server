 ...
 * to work with SharedObjects and streams, as well as connections and scheduling services. <br>
 * ApplicationAdapter is an application level IScope. To handle streaming processes in your
 * application you should implement {@link IStreamAwareScopeHandler} interface and implement
 * handling methods. <br>
 * Application adapter provides you with useful event handlers that can be used to intercept
 ...
 * methods on Client object at server side, Red5 offers much more convenient way to add methods <br>
 ...
 * }
 * </code> <br>
 * This method added to ApplicationAdapter subclass can be called from client side with the
 * following code: <br>
 * <code>
 ...
 * </code> <br>
 * If you want to build a server-side framework this is a place to start and wrap it around
...
public class MultiThreadedApplicationAdapter extends StatefulScopeWrappingAdapter
   ...
   * @return <code>
   * true
   * </code> if connect is successful, <code>
   * false
   * </code> otherwise
   */
  @Override
   ...
   * @return <code>
   * true
   * </code> if scope can be started, <code>
   * false
   * </code> otherwise. See {@link AbstractScopeAdapter#start(IScope)} for details.
   */
  @Override
          ...
          if (desc.getMethod() != null) {
            ...
            if (desc.getMethodReturnType() == null) {
              ...
              if (returnClass instanceof IRed5PluginHandler) {
                ...
                if (props != null) {
                  Method setProps =
                      returnClass.getClass().getMethod("setProperties", new Class[] {Map.class});
   ...
   * created after call from a SWF movie) to the application. <br>
   * You override this method to pass additional data from client to server application using <code>
   ...
   *
   * @param conn Connection object
   * @param params List of parameters after connection URL passed to <code>
   * NetConnection.connect
   * </code> method.
   * @return Boolean value
   */
  ...
  public void measureBandwidth(IConnection conn) {
    // dummy for now, this makes flv player work they dont wait for connected status they wait for
    // onBWDone
   ...
   * instance of RSO at client-side, use <code>
   * SharedObject.getRemote()
   * </code> . SharedObjects can be persistent and transient. Persistent RSO are stateful, i.e.
   * store their data between sessions. If you need to store some data on server while clients go
   * back and forth use persistent SO (just use <code>
   * true
   * </code> ), otherwise prefer usage of transient for extra performance.
   *
  ...
  /**
   * Returns the registered "provider" scope service.
   *
        ...
        providerService =
            (IProviderService)
                ScopeUtils.getScopeService(
                    ...
                    Class.forName("org.red5.server.stream.ProviderService"));
      } catch (ClassNotFoundException e) {
        log.warn("Default provider service class not found", e);
        providerService =
            (IProviderService) ScopeUtils.getScopeService(scope, IProviderService.class);
      }
    }
   ...
   * from server stream name. Server stream name is just an ID assigned by Red5 to every created
   * stream. Broadcast stream name is the name that is being used to subscribe to the stream at
   * client side, that is, in <code>
   * NetStream.play
   * </code> call.
   *
  ...
  public ISubscriberStream getSubscriberStream(IScope scope, String name) {
    ...
    ISubscriberStreamService service =
        (ISubscriberStreamService)
            ScopeUtils.getScopeService(
                scope, ISubscriberStreamService.class, StreamService.class, false);
  ...
  public String addScheduledJob(int interval, IScheduledJob job) {
    if (schedulingService == null) {
      schedulingService =
          (ISchedulingService)
              ScopeUtils.getScopeService(
                  scope, ISchedulingService.class, JDKSchedulingService.class, false);
    }
  ...
  public double getStreamLength(String name) {
    double duration = 0;
    File file = getProviderService().getVODProviderFile(scope, name);
  ...
  /**
   * Start transmission notification from Flash Player 11.1+. This command asks the server to
   * transmit more data because the buffer is running low.
   *
   * <p>http://help.adobe.com/en_US/flashmediaserver/devguide/WSd391de4d9c7bd609-569139412a3743e78e-8000.html
   *
   * @param bool boolean
   * @param num number
   */
  ...
  /**
   * Stop transmission notification from Flash Player 11.1+. This command asks the server to suspend
   * transmission until the client sends a startTransmit event because there is enough data in the
   * buffer.
   */
  ...
  public void streamBroadcastClose(IBroadcastStream stream) {
    ...
    if (conn != null) {
      ...
      log.info(
          "W3C x-category:stream x-event:unpublish x-sname:{} x-file-length:{} x-name:{}",
          new Object[] {stream.getName(), publishDuration, stream.getPublishedName()});
    }
  ...
  public void streamPlayItemPlay(ISubscriberStream stream, IPlayItem item, boolean isLive) {
    // log w3c connect event
    log.info(
        "W3C x-category:stream x-event:play c-ip:{} x-sname:{} x-name:{}",
        new Object[] {
  ...
  public void streamPlayItemStop(ISubscriberStream stream, IPlayItem item) {
    ...
    if (log.isInfoEnabled()) {
      ...
      if (stream instanceof PlaylistSubscriberStream) {
        // converted to seconds
        playDuration =
            (System.currentTimeMillis() - ((PlaylistSubscriberStream) stream).getCreationTime())
                / 1000;
      }
      long playItemSize = -1;
      String playItemName = "";
      if (item != null) {
        ...
        IProviderService providerService =
            (IProviderService) scope.getContext().getBean(IProviderService.BEAN_NAME);
        if (providerService != null) {
  ...
  public void streamPlayItemPause(ISubscriberStream stream, IPlayItem item, int position) {
    // log w3c connect event
    log.info(
        "W3C x-category:stream x-event:pause c-ip:{} x-sname:{}",
  ...
  public void streamPlayItemResume(ISubscriberStream stream, IPlayItem item, int position) {
    // log w3c connect event
    log.info(
        "W3C x-category:stream x-event:unpause c-ip:{} x-sname:{}",
        Red5.getConnectionLocal().getRemoteAddress(),
        stream.getName());
  }
  ...
  public void streamPublishStart(IBroadcastStream stream) {
    ...
    log.info(
        "W3C x-category:stream x-event:publish c-ip:{} x-sname:{} x-name:{}",
        new Object[] {
  ...
  public void streamRecordStart(IBroadcastStream stream) {
    ...
    IConnection connection = Red5.getConnectionLocal();
    log.info(
        "W3C x-category:stream x-event:record-start c-ip:{} x-sname:{} x-file-name:{}",
        new Object[] {
          connection != null ? connection.getRemoteAddress() : "0.0.0.0",
          stream.getName(),
  ...
  public void streamRecordStop(IBroadcastStream stream) {
    ...
    IConnection connection = Red5.getConnectionLocal();
    log.info(
        "W3C x-category:stream x-event:record-stop c-ip:{} x-sname:{} x-file-name:{}",
        new Object[] {
          connection != null ? connection.getRemoteAddress() : "0.0.0.0",
          stream.getName(),
          stream.getSaveFilename()
  ...
  public void streamSubscriberStart(ISubscriberStream stream) {
    // log w3c connect event
    log.info(
        "W3C x-category:stream x-event:play c-ip:{} x-sname:{}",
        Red5.getConnectionLocal().getRemoteAddress(),
        stream.getName());
  }
