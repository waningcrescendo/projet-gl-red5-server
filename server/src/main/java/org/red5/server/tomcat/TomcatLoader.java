 ...
 *
 * <p>http://tomcat.apache.org/tomcat-8.5-doc/api/index.html
 *
 * @author Paul Gregoire (mondain@gmail.com)
 */
@ManagedResource(objectName = "org.red5.server:type=TomcatLoader", description = "TomcatLoader")
public class TomcatLoader extends LoaderBase
  ...
  static {
    ...
    //     at
    // org.apache.catalina.authenticator.AuthenticatorBase.getJaspicProvider(AuthenticatorBase.java:1140)
    //     at org.apache.catalina.authenticator.AuthenticatorBase.invoke(AuthenticatorBase.java:431)
    //     at org.apache.catalina.core.StandardHostValve.invoke(StandardHostValve.java:140)
    Security.setProperty(
        AuthConfigFactory.DEFAULT_FACTORY_SECURITY_PROPERTY, AuthConfigFactoryImpl.class.getName());
  }

  /** Filters directory content */
  protected static final class DirectoryFilter implements FilenameFilter {
    /**
  ...
  public Context addContext(String contextPath, String docBase, Host host) throws ServletException {
    ...
    log.trace(
        "Context loader (check): {} Context classloader: {}",
        ctx.getLoader(),
        ctx.getLoader().getClassLoader());
    LoaderBase.setRed5ApplicationContext(
        getHostId() + contextPath, new TomcatApplicationContext(ctx));
    return ctx;
  }
  ...
  public void start() throws ServletException {
    ...
    if (hosts != null && !hosts.isEmpty()) {
      ...
      for (Host h : hosts) {
        log.debug(
            "Host - name: {} appBase: {} info: {}", new Object[] {h.getName(), h.getAppBase(), h});
      ...
      for (final Container cont : host.findChildren()) {
        if (cont instanceof StandardContext) {
          ...
          try {
            ctx.resourcesStart();
            log.debug(
                "Context - privileged: {}, start time: {}, reloadable: {}",
                new Object[] {ctx.getPrivileged(), ctx.getStartTime(), ctx.getReloadable()});
            Loader cldr = ctx.getLoader();
            log.debug(
                "Loader delegate: {} type: {}", cldr.getDelegate(), cldr.getClass().getName());
            if (log.isTraceEnabled()) {
            ...
            final String contextConfigLocation =
                servletContext.getInitParameter(CONFIG_LOCATION_PARAM) == null
            ...
            final String parentContextKey =
                servletContext.getInitParameter(LOCATOR_FACTORY_KEY_PARAM) == null
                    ...
                    new Runnable() {
                      public void run() {
                        ...
                        if (applicationContext.containsBean(parentContextKey)) {
                          parentContext =
                              (ApplicationContext) applicationContext.getBean(parentContextKey);
                        } else {
                          log.warn("Parent context was not found: {}", parentContextKey);
                        }
                        // create a spring web application context
                        final String contextClass =
                            servletContext.getInitParameter(CONTEXT_CLASS_PARAM) == null
                        ...
                        try {
                          ...
                          if (appctx.containsBean("realm")) {
                            ...
                            if (contextRealm != null) {
                              ...
                              // when a realm implements our red5 realm, add the app and servlet
                              // contexts
                              ...
                              if (contextRealm instanceof JAASRealm) {
                                ...
                                System.setProperty(
                                    ...
                                    prefix + "WEB-INF/jaas.config");
                              }
                              log.debug(
                                  "Realm info: {} path: {}",
                                  contextRealm,
                                  ((RealmBase) contextRealm).getRealmPath());
                            }
                          }
                          appctx.start();
                        } catch (Throwable e) {
                          throw new RuntimeException(
                              "Failed to load webapplication context class", e);
                        }
                      }
                    });
            // see if everything completed
            log.debug(
                "Context: {} done: {}", servletContext.getContextPath(), appStartTask.isDone());
          } catch (Throwable t) {
            log.error(
                "Error setting up context: {} due to: {}",
  ...
  public boolean startWebApplication(String applicationName) throws ServletException {
    ...
    try {
      ...
      // get the (spring) config file path
      final String contextConfigLocation =
          servletContext.getInitParameter("contextConfigLocation") == null
              ? defaultSpringConfigLocation
              : servletContext.getInitParameter("contextConfigLocation");
      log.debug("Spring context config location: {}", contextConfigLocation);
      // get the (spring) parent context key
      final String parentContextKey =
          servletContext.getInitParameter("parentContextKey") == null
              ? defaultParentContextKey
      ...
      // create a thread to speed-up application loading
      Thread thread =
          new Thread("Launcher:" + servletContext.getContextPath()) {
            public void run() {
              // set current threads classloader to the webapp classloader
              Thread.currentThread().setContextClassLoader(webClassLoader);
              // create a spring web application context
              ...
              if (applicationContext.containsBean(defaultParentContextKey)) {
                parentAppCtx =
                    (ApplicationContext) applicationContext.getBean(defaultParentContextKey);
              ...
              appctx.setServletContext(servletContext);
              // set the root webapp ctx attr on the each servlet context so spring can find it
              // later
              servletContext.setAttribute(
                  WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE, appctx);
              appctx.refresh();
  ...
  public void destroy() throws Exception {
    ...
    AbstractApplicationContext absCtx =
        (AbstractApplicationContext) LoaderBase.getApplicationContext();
    if (absCtx != null) {
