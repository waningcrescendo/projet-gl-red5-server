...
/**
 * An extension of {@link PropertyPlaceholderConfigurer}. Provides runtime additions of properties
 * and wildcard location lookups.
 *
 * <p>Properties can be added at runtime by using the static {@link #addGlobalProperty} before* the
 * bean definition is instantiated in the ApplicationContext. A property added by {@link
 * #addGlobalProperty} will get merged into properties specified by the bean definition, overriding
 * keys that overlap.
 *
 * <p>wildcard locations can be used instead of locations, if both are declared the last will
 * override. Wildcard locations are handled by {@link #setWildcardLocations(String[])}, using {@link
 * PathMatchingResourcePatternResolver} for matching locations. For wildcard locations that matches
 * multiple Properties files, they are merged in by alphabetical filename order.
 *
...
public class ExtendedPropertyPlaceholderConfigurer extends PropertyPlaceholderConfigurer {
  ...
  private static Logger logger =
      LoggerFactory.getLogger(ExtendedPropertyPlaceholderConfigurer.class);
