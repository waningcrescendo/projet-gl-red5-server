...
public class WsHttpUpgradeHandler implements InternalHttpUpgradeHandler {
  ...
  public void init(WebConnection connection) {
    ...
    // Create the frame using the application's class loader so it can pick up application specific
    // config from the ServerContainerImpl
    ...
    try {
      ...
      if (isTrace) {
        ...
        log.trace(
            "WS session pre-ctor - wsRemoteEndpointServer: {}, webSocketContainer: {}, handshakeRequest.getRequestURI: {}, handshakeRequest.getParameterMap: {}, handshakeRequest.getQueryString: {}, handshakeRequest.getUserPrincipal: {}, httpSessionId: {}, negotiatedExtensions: {}, subProtocol: {}, pathParameters: {}, secure: {}, endpointConfig: {}",
      ...
      // negotiatedExtensions, subProtocol, pathParameters, secure, endpointConfig);
      wsFrame =
          new WsFrameServer(
              socketWrapper, upgradeInfo, wsSession, transformation, applicationClassLoader);
      // WsFrame adds the necessary final transformations. Copy the completed transformation chain
      // to the remote end point.
      wsRemoteEndpointServer.setTransformation(wsFrame.getTransformation());
      // get the ws scope manager from user props
      WebSocketScopeManager manager =
          (WebSocketScopeManager) endpointConfig.getUserProperties().get(WSConstants.WS_MANAGER);
      // get ws scope from user props
      WebSocketScope scope =
          (WebSocketScope) endpointConfig.getUserProperties().get(WSConstants.WS_SCOPE);
      ...
      // must be added to the session as well since the session ctor copies from the endpoint and
      // doesnt update
  ...
  public void setSslSupport(SSLSupport sslSupport) {
    // NO-OP. WebSocket has no requirement to access the TLS information associated with the
    // underlying connection.
  }

  /**
   * Check to see if the timeout has expired and process a timeout if that is the case. Note: The
   * name of this method originated with the Servlet 3.0 asynchronous processing but evolved over
   * time to represent a timeout that is triggered independently of the socket read/write timeouts.
   *
   * @param now - The time (as returned by System.currentTimeMillis() to use as the current time to
   *     determine whether the timeout has expired. If negative, the timeout will always be treated
  ...
  public void timeoutAsync(long now) {
    ...
    if (wsSession != null) {
      try {
        ...
        if (conn == null) {
          log.warn(
              "Connection for id: {} was not found in the scope or session: {}",
              wsSession.getId(),
              scope.getPath());
          return;
        }
        ...
        if (now > 0) {
          ...
          log.info(
              "timeoutAsync: {}ms on session id: {} read: {} written: {}",
              checkDelta,
              wsSessionId,
              readBytes,
              writtenBytes);
          ...
          log.debug(
              "Session timeouts - max: {} read: {} write: {}",
              maxIdleTimeout,
              readTimeout,
              writeTimeout);
          if (maxIdleTimeout > 0) {
            if (checkDelta > maxIdleTimeout
                && (readBytes == lastReadBytes || writtenBytes == lastWrittenBytes)) {
