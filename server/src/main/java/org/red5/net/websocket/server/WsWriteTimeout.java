 ...
 * {@link javax.servlet.ServletOutputStream} and {@link javax.servlet.ServletInputStream} so there
 * is no way to set a timeout for writes to the client.
 */
public class WsWriteTimeout implements BackgroundProcess {
    ...
    if (backgroundProcessCount >= processPeriod) {
      ...
      for (WsRemoteEndpointImplServer endpoint : endpoints) {
        if (endpoint.getTimeoutExpiry() < now) {
          ...
          // Endpoints are ordered by timeout expiry so if this point is reached there is no need to
          // check the remaining
          // endpoints
          break;
        }
   ...
   * {@inheritDoc}
   *
   * <p>The default value is 1 which means asynchronous write timeouts are processed every 1 second.
   */
  @Override
