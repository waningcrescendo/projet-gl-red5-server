...
public class WebSocketScopeManager {
              ...
              executor.submit(
                  () -> {
                    ...
                    Thread.currentThread()
                        .setName(String.format("WebSocketPinger@%s", appScopeName));
                    do {
                      scopes.forEach(
                          (sName, wsScope) -> {
                                    ...
                                    wsConn -> {
                                      try {
                                        ...
                                        if (wsConn.isConnected()) {
                                          log.trace(
                                              "pinging ws: {} on scope: {}",
                                              wsConn.getWsSessionId(),
                                              sName);
                                          try {
                                            wsConn.sendPing(PING_BYTES);
                                          } catch (Exception e) {
                                            log.debug(
                                                "Exception pinging connection: {} connection will be closed",
                                                wsConn.getSessionId(),
                                                e);
                                            wsConn.close(
                                          ...
                                          log.debug(
                                              "Removing unconnected connection: {} during ping loop",
                                              wsConn.getSessionId());
  ...
  /**
   * Create a web socket scope. Use the IWebSocketScopeListener interface to configure the created
   * scope.
   *
   * @param path
   */
  ...
  /**
   * Create a web socket scope from a server IScope. Use the IWebSocketScopeListener interface to
   * configure the created scope.
   *
   * @param scope
   */
  ...
  private void notifyListeners(
      WebSocketEvent event, WebSocketScope wsScope, WebSocketConnection wsConn) {
    executor.execute(
        () -> {
  ...
  public static void setWebsocketPingInterval(long websocketPingInterval) {
    if (websocketPingInterval < 0) {
      log.warn(
          "Setting the ping interval to a negative value will disable the internal ping worker");
    }
  ...
  public String toString() {
    return String.format(
        "App scope: %s%nActive rooms: %s%nWS scopes: %s%nWS listeners: %s%n",
        appScope, activeRooms, scopes, scopeListeners);
  }
}
