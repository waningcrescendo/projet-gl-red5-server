...
public class WsHandshakeRequest implements HandshakeRequest {
  ...
  public WsHandshakeRequest(HttpServletRequest request, Map<String, String> pathParams) {
    ...
    for (Entry<String, String[]> entry : originalParameters.entrySet()) {
      newParameters.put(
          entry.getKey(), Collections.unmodifiableList(Arrays.asList(entry.getValue())));
    }
  ...
  /**
   * Called when the HandshakeRequest is no longer required. Since an instance of this class retains
   * a reference to the current HttpServletRequest that reference needs to be cleared as the
   * HttpServletRequest may be reused.
   *
   * <p>There is no reason for instances of this class to be accessed once the handshake has been
   * completed.
   */
  void finished() {
    request = null;
