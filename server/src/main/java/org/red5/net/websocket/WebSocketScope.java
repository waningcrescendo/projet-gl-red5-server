...
/**
 * WebSocketScope contains an IScope and keeps track of WebSocketConnection and
 * IWebSocketDataListener instances.
 *
...
public class WebSocketScope implements InitializingBean, DisposableBean {
  ...
  // this has very few entries, possibly only one, COWAS is fine here and won't incur Comparable
  // requirements
  ...
  public WebSocketConnection getConnectionBySessionId(String id) {
    ...
    Optional<WebSocketConnection> opt =
        conns.stream().filter(conn -> id.equals(conn.getSessionId())).findFirst();
    if (opt.isPresent()) {
      return opt.get();
    }
    return null;
   ...
   *
   * @param listeners list of IWebSocketDataListener
   */
  ...
  public boolean hasListener(Class<?> clazz) {
    return listeners.stream()
        .filter(listener -> listener.getClass().isInstance(clazz))
        .findFirst()
        .isPresent();
  }
