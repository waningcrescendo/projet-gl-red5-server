...
public class WsRemoteEndpointImplServer extends WsRemoteEndpointImplBase {
  ...
  public WsRemoteEndpointImplServer(SocketWrapperBase<?> socketWrapper, UpgradeInfo upgradeInfo) {
    ...
    // Paul: our impl doesnt use the WsServerContainer.wsWriteTimeout due to its type and what may
    // be registered, we use our impl classes
  ...
  protected void doClose() {
    if (handler != null) {
      // close() can be triggered by a wide range of scenarios. It is far simpler just to always use
      // a dispatch than it is to try and track
   ...
   * @param useDispatch Should {@link SendHandler#onResult(SendResult)} be called from a new thread,
   *     keeping in mind the requirements of {@link javax.websocket.RemoteEndpoint.Async}
   */
  private void clearHandler(Throwable t, boolean useDispatch) {
    // Setting the result marks this (partial) message as complete which means the next one may be
    // sent which
    // could update the value of the handler. Therefore, keep a local copy before signalling the end
    // of the (partial)
    // message.
    ...
    if (sh != null) {
      if (useDispatch) {
        ...
        if (endpoint.isRunning() && containerExecutor != null) {
          ...
          // This may not be strictly specification compliant in all cases but during shutdown only
          // close messages are going
          // to be sent so there should not be the issue of nested calls leading to stack overflow
          // as described in bug
          // 55715. The issues with nested calls was the reason for the separate thread requirement
