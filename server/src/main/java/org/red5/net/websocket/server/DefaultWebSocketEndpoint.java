...
public class DefaultWebSocketEndpoint extends Endpoint {
  ...
  /**
   * TODO: Currently, Tomcat uses an Endpoint instance once - however the java doc of endpoint says:
   * "Each instance of a websocket endpoint is guaranteed not to be called by more than one thread
   * at a time per active connection." This could mean that after calling onClose(), the instance
   * could be reused for another connection so onOpen() will get called (possibly from another
   * thread).<br>
   * If this is the case, we would need a variable holder for the variables that are accessed by the
   * Room thread, and read the reference to the holder at the beginning of onOpen, onMessage,
   * onClose methods to ensure the room thread always gets the correct instance of the variable
   * holder.
   */
  @Override
  public void onOpen(Session session, EndpointConfig config) {
    ...
    WebSocketConnection conn =
        (WebSocketConnection) session.getUserProperties().get(WSConstants.WS_CONNECTION);
    if (conn == null) {
  ...
  public void onClose(Session session, CloseReason closeReason) {
    ...
    // getting the sessions user properties on a closed connection will throw an exception when it
    // checks state
    try {
      // get ws connection from session user props
  ...
  public void onError(Session session, Throwable t) {
    ...
    // Most likely cause is a user closing their browser. Check to see if the root cause is EOF and
    // if it is ignore it.
    ...
    if (root instanceof EOFException) {
      ...
      // IOException after close. Assume this is a variation of the user closing their browser (or
      // refreshing very quickly) and ignore it.
