 ...
 * This plugin will be called by Red5 plugin launcher to associate application components with
 * WebSockets.
 *
...
public class WebSocketPlugin extends Red5Plugin {
    ...
    server
        ...
        .forEachRemaining(
            gscope -> {
              ...
              gscope
                  .getBasicScopeNames(ScopeType.APPLICATION)
                  .forEach(
                      appName -> {
                        log.debug("Setting up websocket for {}", appName);
                        IScope appScope =
                            (IScope) gscope.getBasicScope(ScopeType.APPLICATION, appName);
  ...
  public WebSocketScopeManager getManager(String path) {
    ...
    if (parts.length > 1) {
      ...
      String name =
          !"default".equals(parts[1]) ? parts[1] : ((parts.length >= 3) ? parts[2] : parts[1]);
      if (log.isTraceEnabled()) {
  ...
  /**
   * Removes and returns the WebSocketScopeManager for the given scope if it exists and returns null
   * if it does not.
   *
  ...
  public static ServerContainer getWsServerContainerInstance(ServletContext servletContext) {
    ...
    if (containerMap.containsKey(path)) {
      ...
      ServerEndpointConfig.Configurator configurator =
          (ServerEndpointConfig.Configurator) WebSocketPlugin.getWsConfiguratorInstance();
      // check for sub protocols
      log.debug("Checking for subprotocols");
      List<String> subProtocols = new ArrayList<>();
      Optional<Object> subProtocolsAttr =
          Optional.ofNullable(servletContext.getInitParameter("subProtocols"));
      ...
      Optional<Object> crossOpt =
          Optional.ofNullable(servletContext.getAttribute("crossOriginPolicy"));
      if (crossOpt.isPresent() && Boolean.valueOf((String) crossOpt.get())) {
        Optional<String> opt =
            Optional.ofNullable((String) servletContext.getAttribute("allowedOrigins"));
        if (opt.isPresent()) {
          ((DefaultServerEndpointConfigurator) configurator)
              .setAllowedOrigins(opt.get().split(","));
        }
      }
      log.debug("Checking for endpoint override");
      // check for endpoint override and use default if not configured
      String wsEndpointClass =
          Optional.ofNullable((String) servletContext.getAttribute("wsEndpointClass"))
      ...
      try {
        ...
        ServerEndpointConfig serverEndpointConfig =
            ServerEndpointConfig.Builder.create(endpointClass, path)
      ...
      servletContext.addListener(
          new HttpSessionListener() {
            ...
            public void sessionCreated(HttpSessionEvent se) {
              ...
              // Don't trigger WebSocket initialization if a WebSocket Server Container is already
              // present
              if (sc.getAttribute(Constants.SERVER_CONTAINER_SERVLET_CONTEXT_ATTRIBUTE) == null) {
                ...
                DefaultWsServerContainer serverContainer =
                    (DefaultWsServerContainer) WebSocketPlugin.getWsServerContainerInstance(sc);
                // set the container to the context for lookup
                sc.setAttribute(
                    Constants.SERVER_CONTAINER_SERVLET_CONTEXT_ATTRIBUTE, serverContainer);
              }
            }
