...
public class WsFilter implements Filter {
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    sc =
        (DefaultWsServerContainer)
            ctx.getAttribute(Constants.SERVER_CONTAINER_SERVLET_CONTEXT_ATTRIBUTE);
    if (sc == null) {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (appCtx != null) {
      ...
      StatefulScopeWrappingAdapter app =
          (StatefulScopeWrappingAdapter) appCtx.getBean("web.handler");
      // applications scope
      IScope appScope = app.getScope();
      if (appScope != null) {
        log.debug(
            "Application scope: {} ws scope: {}",
            appScope.getName(),
            appScope.getAttribute(WSConstants.WS_SCOPE));
      } else {
    ...
    if (mappingResult == null) {
      // No endpoint registered for the requested path. Let the application handle it (it might
      // redirect or forward for example)
      chain.doFilter(request, response);
      return;
    }
