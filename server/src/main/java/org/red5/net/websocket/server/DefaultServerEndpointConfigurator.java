...
public class DefaultServerEndpointConfigurator extends ServerEndpointConfig.Configurator {
  ...
  public boolean checkOrigin(String originHeaderValue) {
    ...
    if (crossOriginPolicy) {
      ...
      Optional<String> opt =
          Stream.of(allowedOrigins)
              .filter(origin -> "*".equals(origin) || origin.endsWith(originHeaderValue))
              .findFirst();
      // non-match fail
      if (!opt.isPresent()) {
  ...
  public void modifyHandshake(
    ...
    if (manager != null) {
      ...
      if (scope == null) {
        ...
        // parent scope
        IScope appScope =
            Optional.ofNullable(applicationScope).orElse(plugin.getApplicationScope(path));
        IScope parentScope = appScope;
        // room scope
        IScope roomScope = null;
        // create child scopes
        log.debug(
            "Creating child websocket scope of {} for path: {} split: {}",
            applicationScope,
            path,
        ...
        Set<IWebSocketDataListener> listeners =
            ((WebSocketScope) appScope.getAttribute(WSConstants.WS_SCOPE)).getListeners();
      ...
      handshakeModifiers.forEach(
          modifier -> {
            modifier.modifyHandshake(request, response);
          });
    } else {
      log.warn(
          "No websocket manager found for path: {} requested uri: {}",
          path,
