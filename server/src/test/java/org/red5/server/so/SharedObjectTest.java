/*
 * RED5 Open Source Flash Server - http://code.google.com/p/red5/ Copyright 2006-2014 by respective authors (see below). All rights reserved. Licensed under the Apache License,
 ...
 *
 * <p>http://help.adobe.com/en_US/FlashMediaServer/3.5_SS_ASD/WS5b3ccc516d4fbf351e63e3d11a11afc95e-7e63.html
 *
...
public class SharedObjectTest extends AbstractJUnit4SpringContextTests {
  ...
  public void testAttributeBlasting() throws Throwable {
    ...
    int expectedVersion =
        2 + (loops * workerCount); // (start version (1) + first entry (1) + (loops x workerCount))
  ...
  public void testAttributeBlastingWithPrimitive() throws Throwable {
    ...
    // calculate expected version
    // int expectedVersion = 2 + (loops * workerCount); //(start version (1) + first entry (1) +
    // (loops x workerCount))
    // the next assertion will fail due to thread collisions trying to update the same key with the
    // same value
  ...
  public void testAttributeBlastingJSON() throws Throwable {
    ...
    log.info("Pass: {} fail: {}", pass.get(), fail.get());
    // pass+fail should equal loops * workers + workers (since starting the worker causes an extra
    // pass or fail)
    ...
    public Integer call() throws Exception {
      ...
      try {
        ...
        if (so != null) {
          ...
          do {
            Optional<Object> opt =
                Optional.ofNullable(so.getAttribute(AttributeKey.counter.name()));
            if (opt.isPresent()) {
              // get attribute
              int c = (int) opt.get();
              ...
              if (so.setAttribute(AttributeKey.counter, ++c)) {
                ...
                // the reason for most failures is a collision of n threads trying to set the same
                // value
  ...
  private class StreamInfo {
    // {duration=1, preencoded=true, fileName=test5.flv, streamName=test5, record=true,
    // startTime=1558112375249, viewerCount=0, fileSize=0, timestamp=1558112375252}
    int duration;
      ...
      try {
        ...
        if (so != null) {
          ...
          do {
            // get attribute
            StreamInfo info =
                gson.fromJson((String) so.getAttribute(AttributeKey.json), StreamInfo.class);
  ...
  private final ISharedObjectListener singletonSOListener =
      new ISharedObjectListener() {
