...
public class RTMPSClientTest {
  ...
  public void test31() throws InterruptedException {
    TLSFactory.setKeystorePath(
        "/media/mondain/terrorbyte/workspace/github-red5/red5-server/server/src/main/server/conf/server.p12");
    TLSFactory.setTruststorePath(
        "/media/mondain/terrorbyte/workspace/github-red5/red5-server/server/src/main/server/conf/truststore.p12");
    ...
    Thread t =
        new Thread(
            () -> {
              client.connect(
                  PropertiesReader.getProperty("rtmps.server"),
                  Integer.valueOf(PropertiesReader.getProperty("rtmps.port")),
                  PropertiesReader.getProperty("rtmps.app"),
                            ...
                            new IPendingServiceCallback() {
                              ...
                              public void resultReceived(IPendingServiceCall call) {
                                ...
                                if (Boolean.valueOf(PropertiesReader.getProperty("rtmps.live"))) {
                                  ...
                                  client.play(
                                      streamId, PropertiesReader.getProperty("rtmps.name"), -1, -1);
                                } else {
                                  client.ping(Ping.CLIENT_BUFFER, streamId, 4000);
                                  client.play(
                                      streamId, PropertiesReader.getProperty("rtmps.name"), 0, -1);
                                }
                              }
                            });
                      }
