...
public class ClientTest extends RTMPClient {
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    player.setConnectionClosedHandler(
        new Runnable() {
          @Override
          public void run() {
  ...
  public void connect() {
    ...
    connect(
        PropertiesReader.getProperty("server"),
        Integer.valueOf(PropertiesReader.getProperty("port")),
        PropertiesReader.getProperty("app"),
        connectCallback);
  }
  ...
  private IPendingServiceCallback methodCallCallback =
  ...
  private IPendingServiceCallback createStreamCallback =
      new IPendingServiceCallback() {
        @Override
        public void resultReceived(IPendingServiceCall call) {
  ...
  protected void onCommand(RTMPConnection conn, Channel channel, Header header, Notify notify) {
    ...
    Object obj =
        notify.getCall().getArguments().length > 0 ? notify.getCall().getArguments()[0] : null;
